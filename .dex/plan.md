...
- [x] `pyproject.toml` — remove `libsql`, add `psycopg`, `testcontainers`
- [x] `src/stream_of_worship/admin/config.py` — remove Turso, add `database_url`, `get_connection_url()`, `SOW_DATABASE_PASSWORD` support
- [x] `src/stream_of_worship/app/config.py` — remove Turso, add `database_url`, `get_connection_url()`, remove `db_path`, `songsets_db_path`

### 12.2 Schema Layer
- [x] `src/stream_of_worship/admin/db/schema.py` — rewrite ALL_SCHEMA_STATEMENTS for Postgres with `timestamptz`
- [x] `src/stream_of_worship/app/db/schema.py` — rewrite for Postgres with `timestamptz`
- [x] **New:** `src/stream_of_worship/db/postgres_schema.py` (optional unified schema file)

### 12.3 Database Clients
- [x] **New:** `src/stream_of_worship/db/connection.py` — `ConnectionProvider` class
- [x] `src/stream_of_worship/admin/db/client.py` — replace sqlite3/libsql with psycopg via `ConnectionProvider`, update SQL dialect, **remove `reset_database()`**
- [x] `src/stream_of_worship/app/db/read_client.py` — replace sqlite3/libsql with psycopg via `ConnectionProvider`, update SQL dialect
- [x] `src/stream_of_worship/app/db/songset_client.py` — replace sqlite3 with psycopg via `ConnectionProvider`, update SQL dialect, remove `snapshot_db()`
- [x] `src/stream_of_worship/admin/db/models.py` — update `DatabaseStats`, add `_to_str()` coercion in `from_row()` methods
- [x] `src/stream_of_worship/app/db/models.py` — add `_to_str()` coercion in `from_row()` methods

### 12.4 Services
- [x] `src/stream_of_worship/admin/services/sync.py` — **DELETE** or replace with `check_database_connection()`
- [x] `src/stream_of_worship/app/services/sync.py` — **DELETE** or replace with connection checker

### 12.5 CLI Commands
- [x] `src/stream_of_worship/admin/commands/db.py` — remove sync/turso-bootstrap/tokens/**reset** commands, update init/status, add `url` command
- [x] `src/stream_of_worship/app/main.py` — remove sync CLI command, add `db check` command

### 12.6 Application Layer
- [x] `src/stream_of_worship/app/app.py` — update client initialization with `ConnectionProvider`, remove sync service setup
- [x] `src/stream_of_worship/app/services/catalog.py` — update imports, connection handling
- [x] All app screens using `SongsetClient` — verify constructor signature change (now accepts `ConnectionProvider`)
- [x] `src/stream_of_worship/app/screens/songset_list.py` — verify `fromisoformat()` works with `timestamptz`-derived strings

### 12.7 Tests
- [x] `tests/conftest.py` — add `postgres_url` fixture, add `@pytest.mark.integration` marker
- [x] `tests/admin/commands/test_db_commands.py` — rewrite for Postgres/testcontainers, remove reset tests
- [x] `tests/app/services/test_catalog_cross_db.py` — rewrite for unified Postgres DB
- [x] `tests/app/db/test_songset_client.py` — remove `snapshot_db()` tests, update for `ConnectionProvider`
- [x] **New:** `tests/db/test_postgres_clients.py` — integration tests for psycopg clients
- [x] **New:** `tests/db/test_role_permissions.py` — verify app role restrictions
- [x] **New:** `tests/db/test_connection_provider.py` — unit tests for ConnectionProvider retry logic
- [x] **New:** `tests/db/test_model_coercion.py` — unit tests for `_to_str()` datetime→string coercion
...
- [x] This plan is approved by operator (mhuang) (skipped — not automatable)
- [x] Neon roles `sow_admin_rw` and `sow_app` are created with correct privileges (skipped — not automatable)
- [x] Neon pooled connection hostname (`-pooler`) is identified and verified (skipped — not automatable)
- [x] v4 runbook data migration is ready (`01_schema.sql`, `02_load_data.py`, etc.) (skipped — not automatable)
- [x] A staging branch in Neon is available for testing (skipped — not automatable)
- [x] `specs/migration/checklists/cutover_checklist.md` is up to date (skipped — not automatable)
