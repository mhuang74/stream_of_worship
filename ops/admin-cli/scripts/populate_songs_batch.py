...
def get_catalog_ids(album=None):
    ...
    cmd = f"uv run --project ops/admin-cli --python 3.11 --extra admin python -m stream_of_worship.admin.main catalog list --format ids {album_opt}"
...
def get_audio_ids():
    ...
    cmd = "uv run --project ops/admin-cli --python 3.11 --extra admin python -m stream_of_worship.admin.main audio list --format ids"
    success, stdout, stderr = run_command(cmd, timeout=60)
