...
def role_eligibility_counts(
    ...
    return {
        ...
        "valid_closers_h3": sum(
            ...
            and candidate.tempo_bpm is not None and candidate.tempo_bpm <= closing_limit
        ),
...
def _proposal_for_diagnostics(
    ...
    if len(draft.items) > 1:
        ...
        for left, right in zip(draft.items, draft.items[1:]):
            ...
            if transition:
                updated_items.append(right.model_copy(update={
...
def beam_diagnostics(
    ...
    *,
    ...
    relaxed = hard_rule_rejection_counts(sequences, config, matrix, relax_kwargs={"relax_h4": True, "relax_h5": True})
    return {
...
def diagnostic_lines(config: RunConfig, result: dict) -> list[str]:
    ...
    if isinstance(enriched, int) and enriched < config.count:
        lines.append(f"only {enriched} enriched candidates remain for a {config.count}-song set")
    ...
    if isinstance(rejections, dict) and rejections:
        ...
        if generated:
            lines.append(f"beam validation rejected {rejected or 0}/{generated} generated sequences: {counts}")
        else:
    ...
    if isinstance(relaxed, dict):
        ...
        if isinstance(relaxed_rejections, dict) and relaxed_rejections and (
            relaxed_rejected != rejected or relaxed_rejections != (rejections if isinstance(rejections, dict) else {})
        ):
            relaxed_counts = ", ".join(f"{code}={count}" for code, count in sorted(relaxed_rejections.items()))
            if relaxed_generated:
                lines.append(f"beam relaxed-tier fallback would still reject {relaxed_rejected or 0}/{relaxed_generated} sequences: {relaxed_counts}")
            else:
                lines.append(f"beam relaxed-tier fallback rejections by rule: {relaxed_counts}")
