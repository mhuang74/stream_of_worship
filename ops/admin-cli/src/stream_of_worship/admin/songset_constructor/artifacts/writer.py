...
def _proposal_structured_data(proposal: SongsetProposal) -> str:
    ...
    for item in proposal.items:
        ...
        lines.append(f"  {item.position}. {item.title} | {phase_str} | themes: {themes} | key {key} mode {mode} | bpm {bpm}")
    lines.append(f"  Score: f_theme {proposal.score.f_theme:.3f}, f_tempo {proposal.score.f_tempo:.3f}, f_harmony {proposal.score.f_harmony:.3f}, f_diversity {proposal.score.f_diversity:.3f}, total {proposal.score.total:.3f}")
    warnings = ", ".join(proposal.hard_constraint_warnings) if proposal.hard_constraint_warnings else "none"
...
def generate_brief_summaries(config: RunConfig, proposals: list[SongsetProposal]) -> list[str]:
    deterministic = [_deterministic_arc_narrative(p) for p in proposals]
    if not config.llm_enabled or len(proposals) < 2:
        return deterministic
    try:
        from stream_of_worship.admin.songset_constructor.graph.llm import build_chat_model
        ...
        if len(parsed) != len(proposals) or not all(parsed.get(index) for index in range(1, len(proposals) + 1)):
            return deterministic
        return [_truncate_sentences(parsed[index + 1]) for index in range(len(proposals))]
    except Exception:
        return deterministic
...
def _diversity_metrics(proposals: list[SongsetProposal], pool: list[SongCandidate]) -> dict:
    ...
    unique_composers = {by_song[item.song_id].composer for p in proposals for item in p.items if by_song.get(item.song_id) and by_song[item.song_id].composer}
    unique_phases = set()
    for p in proposals:
        for item in p.items:
    ...
    return {
        "total_slots": total_slots, "unique_songs": unique_songs,
        "unique_themes": unique_themes, "unique_composers": unique_composers,
        "unique_phases": unique_phases, "unique_middle_songs": unique_middle_songs,
        "total_middle_slots": total_middle_slots, "middle_reuse_count": middle_reuse_count,
    }
...
def _song_overlap_matrix(proposals: list[SongsetProposal]) -> list[str]:
    ...
    lines = ["| " + " | ".join(headers) + " |", "|" + "|".join("---" for _ in range(count + 1)) + "|"]
    for row in range(count):
        cells = [f"R{row + 1}"]
        for col in range(count):
            cells.append("—" if row == col else str(len(song_sets[row] & song_sets[col])))
...
def _theme_coverage_lines(proposals: list[SongsetProposal]) -> list[str]:
    present = {theme for p in proposals for item in p.items for theme in item.themes if theme in THEMES}
...
def _bottleneck_lines(metrics: dict, proposals: list[SongsetProposal], pool: list[SongCandidate], *, config: RunConfig | None = None) -> list[str]:
    ...
    overused = [(sid, len(pset)) for sid, pset in song_usage.items() if len(pset) > len(proposals) / 2]
    ...
    if total_slots > 0:
        ...
        for p in proposals:
            for item in p.items:
                ...
                if candidate and candidate.composer:
                    composer_counts[candidate.composer] = composer_counts.get(candidate.composer, 0) + 1
        concentrated = [(composer, count) for composer, count in composer_counts.items() if count > total_slots / 3]
        ...
        for composer, count in concentrated:
            ...
            lines.append(f'Composer concentration: composer "{composer}" authored {count}/{total_slots} slots ({percentage}%).')
    return lines
...
def write_pool_csv(path: Path, pool: list[SongCandidate]) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for candidate in pool:
            ...
            writer.writerow({
                "song_id": candidate.song_id, "title": candidate.title, "title_pinyin": candidate.title_pinyin,
                "composer": candidate.composer, "album_name": candidate.album_name, "album_series": candidate.album_series,
...
def build_review_report(*, config: RunConfig, proposals: list[SongsetProposal], pool: list[SongCandidate], trace: list[dict], chat: Any | None = None, fallback_note: str | None = None) -> str:
    generated_at = datetime.now(UTC).isoformat()
    ...
    try:
        ...
        return _fallback_review_report(config, proposals, pool, trace, generated_at, "LLM report generation failed; fallback report used.")


def _review_payload(config: RunConfig, proposals: list[SongsetProposal], pool: list[SongCandidate], trace: list[dict], generated_at: str) -> dict[str, Any]:
    config_data = config.to_dict()
    return {"config": config_data, "run_summary": {"run_id": config.thread_id, "generated_at": generated_at, "requested_song_count": config.count, "proposals": config.proposals, "pool_size": len(pool), "flags": _config_flags(config)}, "candidate_pool": _candidate_pool_summary(pool, trace), "trace_summaries": _trace_stage_summaries(trace), "validation_events": _validation_events(trace), "proposals": [proposal.model_dump(mode="json") for proposal in proposals]}
...
def _fallback_review_report(config: RunConfig, proposals: list[SongsetProposal], pool: list[SongCandidate], trace: list[dict], generated_at: str, fallback_note: str | None = None) -> str:
    lines = _fallback_title_and_findings(config, proposals, pool, trace).splitlines()
    if fallback_note:
        lines.extend(["", f"> {fallback_note}"])
    lines.extend(["", "## Run Summary", "", f"- Run ID: {config.thread_id}", f"- Generated: {generated_at}", f"- Requested song count: {config.count}", f"- Top-k: {config.proposals}", f"- Pool size: {len(pool)}", f"- Relevant flags: {_format_flags(_config_flags(config))}", "", "## What Was Done", "", "- Loaded the catalog pool from the configured read-only source.", "- Enriched candidates with phase and theme metadata, dropping songs without enough tempo/key data.", "- Built compatible transition candidates and seeded ranked sequences.", "- Finalized ranked proposals from beam candidates and validation state.", "- Wrote proposal, report, pool, trace, and review artifacts.", "", "## How Filters Were Applied", "", _filters_summary(config, proposals, pool, trace), ""])
...
def _fallback_title_and_findings(config: RunConfig, proposals: list[SongsetProposal], pool: list[SongCandidate], trace: list[dict]) -> str:
    ...
    findings = [
        f"{len(proposals)} proposal{'s' if len(proposals) != 1 else ''} were generated." if proposals else "No valid proposals were generated.",
        f"Final proposals came from {origin}.",
        f"Relaxation or constraint warnings: {', '.join(warnings) if warnings else 'none reported'}.",
...
def _candidate_pool_summary(pool: list[SongCandidate], trace: list[dict]) -> dict[str, Any]:
    bpm_values = [round(candidate.tempo_bpm) for candidate in pool if candidate.tempo_bpm is not None]
    phase_counts = Counter(candidate.phase for candidate in pool)
    secondary_phase_counts: Counter[int] = Counter()
    for c in pool:
        for p in c.secondary_phases:
            secondary_phase_counts[p] += 1
    return {"total": len(pool), "phase_counts": dict(sorted(phase_counts.items())), "secondary_phase_counts": dict(sorted(secondary_phase_counts.items())), "known_bpm_count": len(bpm_values), "missing_bpm_count": len(pool) - len(bpm_values), "common_bpm_values": Counter(bpm_values).most_common(8), "dropped_counts": _latest_trace_data(trace, "enrich_pool")}
...
def _trace_stage_summaries(trace: list[dict]) -> list[dict[str, Any]]:
    ...
    for entry in trace:
        ...
        summaries.append({"node": entry.get("node"), "event": entry.get("event"), "iteration": entry.get("iteration"), "data": {key: value for key, value in data.items() if key != "prompt"}})
    return summaries


def _validation_events(trace: list[dict]) -> list[dict[str, Any]]:
    return [entry.get("data", {}) for entry in trace if entry.get("node") == "validate_score" and entry.get("event") == "validation"]
...
def _config_flags(config: RunConfig) -> dict[str, Any]:
    return {"llm_enabled": config.llm_enabled, "llm_judge": config.llm_judge, "interactive_review": config.interactive_review, "auto_relax": config.auto_relax, "relax_h1": config.relax_h1, "relax_h2_bpm": config.relax_h2_bpm, "relax_h3_bpm": config.relax_h3_bpm, "relax_h4": config.relax_h4, "relax_h4_bpm": config.relax_h4_bpm, "relax_h5": config.relax_h5, "relax_h5_cfd": config.relax_h5_cfd, "intimate": config.intimate, "hymnal_mode": config.hymnal_mode, "season": config.season}


def _format_flags(flags: dict[str, Any]) -> str:
    enabled = [f"{key}={value}" for key, value in flags.items() if value not in (False, None, [], {})]
    return ", ".join(enabled) if enabled else "none"


def _filters_summary(config: RunConfig, proposals: list[SongsetProposal], pool: list[SongCandidate], trace: list[dict]) -> str:
    enrich = _latest_trace_data(trace, "enrich_pool")
    validation = _validation_events(trace)
    warnings = _proposal_warnings(proposals)
    parts = [f"Enrichment output contains {len(pool)} candidates; dropped={enrich.get('dropped', 0)}.", f"Validation events recorded: {len(validation)}.", f"Final relaxed warning flags: {_format_flags(_config_flags(config))}."]
    failures = []
...
def _proposal_origin(proposals: list[SongsetProposal]) -> str:
    ...
    if all(p.llm_origin for p in proposals):
        return "LLM-origin proposals"
    if any(p.llm_origin for p in proposals):
...
def _proposal_warnings(proposals: list[SongsetProposal]) -> list[str]:
    return sorted({warning for proposal in proposals for warning in proposal.hard_constraint_warnings})
