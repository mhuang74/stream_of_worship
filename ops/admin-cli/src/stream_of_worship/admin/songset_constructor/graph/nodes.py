"""LangGraph node implementations (v3 — in-DB scoring)."""
...
def write_enrichment_report(state: ConstructorState) -> dict:
    ...
    output_dir = Path(config.output_dir) if config.output_dir else Path("output") / "songset_constructor"
...
def llm_plan(state: ConstructorState) -> dict:
    ...
    prompt = (
        f"Select a {config.count}-song Chinese worship set using only these hash prefixes.\n"
        f"Return exactly {config.count} items.\n\n{_pool_prompt(state)}"
    )
...
def llm_refine(state: ConstructorState) -> dict:
    ...
    prompt = (
        f"Repair this {config.count}-song draft using only known hash prefixes.\n"
...
def route_after_beam(state: ConstructorState) -> str:
    if not state["config"].llm_enabled or not state.get("beam_candidates"):
