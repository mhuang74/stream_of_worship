# Implementation Plan: Remote-Write + Local-Read Architecture

**Date:** 2026-05-05
**Branch:** reconcile_fixes (current) or new branch
**Supersedes:** `specs/fix_walconflict_sync_error_opus.md`, `specs/simplify_turso_sync_publish_update.md`
...
| `src/stream_of_worship/admin/commands/db.py` | Update `db sync` command; add `db pull`, `db update` commands |
...
**No changes to**: `admin/db/models.py`, `admin/db/schema.py` (COLUMN_MIGRATIONS list), `app/services/sync.py`
...
**Note**: `_execute_remote()` for PRAGMA returns the result set; the column name is at index 1 of each row in `result["rows"]`. Verify the exact HTTP API response structure during implementation (it may use column objects instead of positional arrays).
...
The `db sync` command maps to `sync()` which in the new model is pull-only. Add named commands:

| Command | Behavior | Old behavior |
|---------|----------|-------------|
| `db pull` | `client._sync_replica(fatal=False)` + report | New |
| `db publish` | No-op (writes already on remote). Just does `db pull` to confirm. | Proposed in `simplify_turso_sync_publish_update.md` |
| `db update` | Delete local DB + `client.sync()` (force re-pull) | Recovery path |
| `db sync` | Alias → `db pull` (deprecated, keep for backward compat) | Was bidirectional |
...
1. **PRAGMA table_info via HTTP API**: The `/v2/pipeline` returns results in `{"rows": [...], "cols": [...]}`. Each row is a list of typed value objects, not plain arrays. Confirm the exact format for PRAGMA responses before implementing `apply_column_migrations_remote()`. The column name is at `cols[1].name` (not `row[1]` as noted above — this depends on the actual API response shape). **Verify by making a test call during implementation.**

2. **`update_sync_metadata()` — keep local or go remote?**: Currently writes to the local replica only (admin-internal metadata). In the new model, this is fine as local-only since `sync_metadata` isn't used by the app. But if we want `last_sync_at` visible to any future admin on a different machine, it should also go remote. **Decision**: Keep local-only for now.

3. **`requests` import in `client.py`**: Currently `requests` is only in `admin` extras. Confirm it's available in the `admin` extra's dependency list in `pyproject.toml` before importing. If not, `uv add --extra admin requests` is needed (though it's almost certainly already there given `analysis.py` uses it).
