# Implementation Plan: Component Feature Extraction Performance Optimization (v2)

> **Date:** 2026-08-10
> **Branch:** TBD
> **Spec ID:** `component-feature-extraction-perf-v2`
> **Supersedes:** `component-feature-extraction-perf-v1`

---

## Summary of Changes from v1

This v2 plan incorporates review feedback on v1:

1. **Phase 3 (onset/RMS caching) merged into Phase 1** — the `GlobalFeatures` slicing strategy already achieves this; a separate phase was redundant.
2. **Phase 4 (fuzzy matching optimization) dropped** — the 244s bottleneck is ~240s of librosa computation. Fuzzy matching is pure Python string comparison and negligible. The optimization added complexity for no measurable gain.
3. **`detect_key_fulltrack` fallback added** — v1's `_detect_key_from_precomputed_chroma` returns `(None, None)` when no windows pass thresholds, leaving `component.key = None`. The original `detect_key_segment_vote` falls back to `detect_key_fulltrack` internally (analyzer.py:169). v2 preserves this fallback.
4. **`sr` parameter fixed** — v1's `_precompute_global_features` accepted `sr: int` but `extract_components` loads audio with `sr=None` (native sample rate). v2 passes `sr=None` to preserve the existing behavior.
5. **Chroma equivalence clarified** — v1's risk table stated full-track chroma sliced to component range "may give slightly different key results." This is overstated: CQT is a sliding window transform, so `chroma[:, start_frame:end_frame]` is near-identical to computing `chroma_cqt` on the slice. The risk is minimal.
6. **Voting simplification justified** — v1's `_detect_key_from_precomputed_chroma` drops the weighted voting logic from `detect_key_segment_vote`. This is valid because each component call passes a single segment (`segments=[{"start": 0.0, "end": segment_duration}]`), so there is only one window and voting is a no-op.
...
4. Per-component `onset_strength` and `rms` are inherently cached by the
   `GlobalFeatures` slicing strategy — no separate caching logic needed.
...
audio, returning a dataclass of pre-computed arrays:
...
def _precompute_global_features(
    ...
    # CQT is a sliding window transform, so chroma[:, start_frame:end_frame]
    # is near-identical to computing chroma_cqt on the slice directly.
...
- **Key**: Call `_detect_key_from_precomputed_chroma()` once, get both `key` and
  ...
  duplicate `detect_key_segment_vote` call.** If the function returns
  `(None, None)`, fall back to `detect_key_fulltrack(gf.y_harmonic, gf.sr)`
  once (see Phase 2 for details).
...
The sliced onset_env and RMS are cheap numpy array views — computed once and
reused for multiple feature fields within the function. No separate caching
logic is needed; the `GlobalFeatures` slicing strategy inherently eliminates
the redundant `onset_strength` and `rms` calls identified as root causes #3
and #4.
...
The `_precompute_global_features()` call should be wrapped in
`loop.run_in_executor()` to avoid blocking the event loop, matching the
existing pattern for `_load_audio()`.

---

## Phase 2: Eliminate duplicate key detection call with fulltrack fallback
...
# Fallback: if no windows passed thresholds, use full-track key detection
# on the pre-computed harmonic signal (mirrors detect_key_segment_vote's
# internal fallback to detect_key_fulltrack at analyzer.py:169).
if key is None:
...
**Note on fallback cost:** `detect_key_fulltrack` computes its own `chroma_cqt`
internally (analyzer.py:80). However, this fallback only triggers when
`_detect_key_from_precomputed_chroma` returns `(None, None)` — i.e., when no
component windows pass the RMS/variance/margin thresholds. In practice this is
rare (most components have sufficient energy and chroma variance). When it does
trigger, it runs on `gf.y_harmonic` (the full track), not per-slice, so it's a
single call, not 17. The cost is one additional `chroma_cqt` computation in the
worst case, which is acceptable.

**Alternative considered:** We could modify `detect_key_fulltrack` to accept
pre-computed chroma, but that would change the `analyzer.py` API (which v1
explicitly avoided). The fallback approach keeps `analyzer.py` unchanged.

---

## Phase 3: Add timing instrumentation

**Complexity:** S

**File:** `ops/analysis-service/src/sow_analysis/workers/components.py`

### 3.1 Add per-stage timing logs
...
### 4.1 Update `compute_component_features` tests
...
Add a test helper to construct a `GlobalFeatures` from a synthetic audio signal:
...
def _make_global_features(y: np.ndarray, sr: int, **overrides) -> GlobalFeatures:
    ...
    defaults = dict(
        y=y, sr=sr, duration=float(librosa.get_duration(y=y, sr=sr)),
        onset_env=onset_env, onset_frames=np.arange(len(onset_env)),
        onset_times=onset_times,
        rms=rms, rms_times=rms_times,
        y_harmonic=y_harmonic, chroma=chroma,
        drums_y=None, drums_onset_env=None,
        drums_rms=None, drums_rms_times=None,
        vocals_y=None,
    )
...
### 4.2 Add test for `_precompute_global_features()`
...
### 4.3 Add test for `_detect_key_from_precomputed_chroma()`

Add a test that verifies:
- Returns `(key, margin)` tuple
- Key is a valid note name (e.g., "G", "C#")
- Margin is a float or None
- Returns `(None, None)` for segments shorter than 8.0 seconds
- Returns `(None, None)` when chroma variance is below threshold

### 4.4 Add test for `detect_key_fulltrack` fallback

Add a test that verifies:
- When `_detect_key_from_precomputed_chroma` returns `(None, None)`,
  `compute_component_features` falls back to `detect_key_fulltrack`
- `component.key` is populated from the fallback result
- `component.key_confidence` is computed from the fallback's `score_margin`

### 4.5 Add performance regression test
...
| `hpss` calls | 34 | 1 | 34× |
| `chroma_cqt` calls | 34 | 1 (+ 1 fallback in rare cases) | ~34× |
...
| File | Change |
|---|---|
| `ops/analysis-service/src/sow_analysis/workers/components.py` | Add `GlobalFeatures` dataclass, `_precompute_global_features()`, `_detect_key_from_precomputed_chroma()`. Refactor `compute_component_features()` and `extract_components()`. Add `detect_key_fulltrack` fallback. Add timing logs. |
| `ops/analysis-service/tests/test_components.py` | Update tests for new `compute_component_features` signature. Add tests for new helpers, fallback, and perf regression. |
...
| Risk | Mitigation |
|---|---|
| Full-track chroma sliced to component range may differ from per-slice chroma | Minimal risk — CQT is a sliding window transform, so `chroma[:, start_frame:end_frame]` is near-identical to computing `chroma_cqt` on the slice. Add a comparison test to verify. |
| `_detect_key_from_precomputed_chroma` bypasses `detect_key_segment_vote` fallback to `detect_key_fulltrack` | **Addressed in v2:** Added explicit `detect_key_fulltrack(gf.y_harmonic, gf.sr)` fallback when the function returns `(None, None)`. This mirrors the original fallback at analyzer.py:169. |
| Memory: holding `y`, `y_harmonic`, `chroma`, `onset_env`, `rms` simultaneously | For a 4-minute song at 44.1kHz: `y` ≈ 10.5M samples (~84MB float64), `y_harmonic` ≈ 84MB, `chroma` ≈ (12, ~20k) ≈ 1.9MB. Total ~170MB — acceptable for the analysis service container. |
| Stems loading adds memory if `--use-stems` | Already the case in current code. No change. |
| `detect_key_fulltrack` fallback computes its own `chroma_cqt` (redundant with pre-computed) | Acceptable — fallback is rare (only when no component windows pass thresholds). Modifying `detect_key_fulltrack` to accept pre-computed chroma would change `analyzer.py` API, which this plan avoids. |
