# Promote Musical Key & BPM Filters to Top-Level Dropdowns (v1)
...
- **BPM Range**
  ...
  - One selected: band label, e.g. `"Slow (< 90)"` (label + range text, same format the chips currently show)
  - Two/three selected: comma-joined labels, e.g. `"Slow (< 90), Fast (>= 120)"`
  - Range text helper: reuse the existing `rangeText` logic from `SharedFilters.tsx` lines 140-148, extracted into a small pure helper in `lib/constants.ts` or a sibling helper module.
...
2. **`delivery/webapp/src/components/songset/BpmRangeMultiSelect.tsx`**
   ...
   - Iterates over `BPM_BAND_KEYS` from `@/lib/constants` and renders each item with the same `"label (rangeText)"` format as the existing chips.
   - Implements the trigger summary logic described in Target UX.
...
- Keep the existing top-level "Clear all" affordance only if Album keeps its own (which it does, inside the dropdown). Remove the panel-level Clear button. If the existing `onClearFilters` is meant to clear **all three** filters at once, keep a single page-level "Clear all" button somewhere visible (e.g., inline at the end of the filter row) — confirm with existing test expectations before removing.
...
- `BpmRangeMultiSelect`
  ...
  - Selecting one band → summary shows `"Slow (< 90)"` style text.
  - Selecting multiple → comma-joined, e.g. `"Slow (< 90), Fast (>= 120)"`.
...
5. Refactor `SharedFilters.tsx` to drop Advanced panel and render the three dropdowns as siblings.
6. Update `BrowseSheet.tsx`: change `selectedBpm` to `BpmBandKey[]`, update both search paths' serialization.
7. Update `SemanticSearch.tsx` JSON body shape.
8. Update/add tests; fix broken test IDs.
9. `pnpm --filter sow-webapp lint && pnpm --filter sow-webapp test`.
10. Manual smoke check via `pnpm --filter sow-webapp dev`.

## Risks & Open Items

- **Backend multi-value `bpmRange`**: must verify current parser behavior before assuming it needs changes.
- **"Clear all" affordance**: confirm whether a page-level clear-all button is expected by UX or existing tests (current panel-level Clear resets all three). Decision: keep a page-level inline "Clear all" at the end of the filter row to preserve that affordance.
