...
1. `delivery/webapp/src/app/share/[token]/play/controller/page.tsx` — two edits:
...
4. **Lyrics pull-up, keyboard, on-screen controls** (shared via `<ControllerPlayer>`, smoke-test only) —
...
5. **Exit behavior** — Inside the share controller, press the Back button (top-left `ArrowLeft`).
...
- **Presigned URL lifetime.** The share controller already receives a presigned `mp4Url` from `/api/share/{token}` (line 59) — no API change is needed. The receiver will consume this same URL via `?v=`. **Verify** that the share-token endpoints mint a sufficiently long-lived presigned URL for a full worship service. The songsets receiver uses a 4-hour TTL via `?cast=true` on `/api/signed-url`; verify `/api/share/{token}` returns a similarly long-lived URL (otherwise the video may stop partway through a service when the presigned URL on `?v=` expires).
