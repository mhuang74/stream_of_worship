# Implementation Plan: Chorus-Based Song Component Metadata

## Overview

Enhance the **Analysis Service**, **PostgreSQL Database Schema**, and **Admin CLI** to generate, persist, and surface per-component structural data (chorus entry/exit, verse loop target) with per-component audio features, so that transition logic can use chorus sections as jumping points between songs.

This plan implements the normalized long-format `song_components` table (one row per detected component instance) specified in `reports/chorus_component_metadata_impl_plan_prompt.md`.

---

## allin1 Evaluation: Hybrid Chorus Identification Strategy

### Problem

The `allin1` library runs Demucs source separation + a structural segmentation neural network. On a laptop CPU this costs **30–60 minutes per song**. The prompt observes that "chorus can be easily identified via lyrics repetition," suggesting we may not need allin1 at all.

### Findings

1. **allin1 sections are already cached** for every song with `analysis_status='completed'`. The `recordings.sections` column holds the JSON list `[{"label","start","end"}]`, and the full `analysis.json` (including `beats`, `downbeats`, `sections`) is mirrored to R2 at `{hash_prefix}/analysis.json`. **Extracting components from already-analyzed songs costs nothing** — the expensive Demucs run already happened.

2. **LRC is optional** (`lrc_status` tracks `pending`/`processing`/`completed`/`failed`), but when present it is a timestamped line list (`LRCLine{time_seconds, text}`). Choruses repeat verbatim or near-verbatim across occurrences, making them detectable via repeated-line-group clustering in **<1 second** per song.

3. **Per-component audio features** (groove_density, backbeat_strength, energy_level, per-segment BPM/key) require librosa on the audio slice regardless of identification method. This is fast (~5–15 s/song) and does NOT need allin1 or Demucs.

4. **No existing repetition/chorus detection** exists — only a stub `_detect_sections()` returning `unknown`.
...
**Why not lyrics-only?** It discards reliable, already-computed allin1 structural labels for the completed-analysis catalog, and fails entirely for the (common) case where LRC is missing. The hybrid approach maximizes coverage at minimum cost.

**Benefit of hybrid:** Backfill of the entire completed-analysis catalog is instant (tier 1). New songs with LRC get components in seconds (tier 2). Songs with neither sections nor LRC are skipped gracefully.

### Where Component Extraction Runs

Per-component feature computation (librosa) requires the **audio slice**. Per the architectural separation rule (Admin CLI never imports ML libraries), all identification AND feature computation happens in the **Analysis Service** via a new `COMPONENT_ANALYSIS` job type. The Admin CLI submits the job (passing cached `sections`/`lrc_content` to avoid re-computation), polls for completion, and persists the returned component rows to `song_components`.

---

## Phase 0: Schema & Models

**Goal:** Create the `song_components` table, `SongComponent` dataclass, column constants, indexes, trigger, and DB client methods.
...
Re-export the new constants and insert them into `ALL_SCHEMA_STATEMENTS` in the catalog section (after `CREATE_RECORDINGS_UPDATE_TRIGGER`):
...
**Goal:** Create `workers/components.py` with chorus identification (two strategies: allin1-section labels and lyrics-repetition clustering) and per-component audio feature computation.
...
class ComponentInstance:
    ...
    energy_level: Optional[float] = None
    confidence: Optional[float] = None
    source: str = ""            # 'allin1_sections' | 'lyrics_repetition'
```
...
- Multiple consecutive 'verse' sections before the first chorus → take the last verse section before the chorus (closest to it).
...
def identify_from_lyrics_repetition(
    ...
    3. For window sizes w = 2..min(12, N//2):
       ...
       b. For each window start i, compute a signature = tuple(normalized_texts[i:i+w]).
       c. Group window-start indices by exact-signature match; record groups with
          >= 2 occurrences as candidates.
...
**Fuzzy matching note:** `rapidfuzz` is already a dependency (used in `services/canonical_snap.py`). Import `from rapidfuzz import fuzz`.

**LRC parser note:** The analysis service does not currently have an LRC parser. Add a minimal `parse_lrc_lines(content: str) -> list[tuple[float, str]]` in this module (regex-based, same pattern as admin-cli's `parse_lrc`). Keep it minimal — no metadata tag preservation needed for component detection.
...
async def extract_components(
    ...
    3. Else: attempt to download analysis.json from R2 (hash_prefix/analysis.json)
       → if found, use its sections (tier 1 lateness).
    4. Else: attempt to download lyrics.lrc from R2 → identify_from_lyrics_repetition()
    5. If all fail → ([], 'none').
...
**Goal:** Add the `COMPONENT_ANALYSIS` job type, request/result models, route endpoint, queue processing, and cache strategy.
...
Update `job_to_response()` to pass the new `components` and `component_source` fields into `JobResult`:
...
Add semaphore consideration: Component analysis is librosa-only (like fast_analyze). Reuse `_fast_analyze_semaphore` or add a dedicated `_component_semaphore`. **Recommendation: reuse `_fast_analyze_semaphore`** since both are librosa-CPU-bound and should coordinate.
...
def get_component_result(self, content_hash: str) -> Optional[dict]:
    """Check if component analysis result exists in cache."""
...
def save_component_result(self, content_hash: str, result: dict) -> Path:
    """Save component analysis result to cache."""
    hash_prefix = self._get_hash_prefix(content_hash)
    cache_file = self.cache_dir / f"{hash_prefix}_components.json"
...
The `extract_components()` function checks cache first (unless `force=True`), mirroring `analyze_audio`'s pattern.

### 2.5 `ops/analysis-service/src/sow_analysis/storage/r2.py`

Add upload method for component results:
...
async def download_lrc_text(self, hash_prefix: str) -> Optional[str]:
    ...
    # Uses check_exists + download to temp file + read_text
```
...
**Goal:** Submit component analysis jobs, persist results to `song_components`, display component metadata in the CLI.
...
Update `_parse_job_response()` to parse the new `components` and `component_source` fields into `AnalysisResult`. Add fields to the `AnalysisResult` dataclass:
...
3. Downloads LRC content from R2 via `R2Client.download_lrc_content(hash_prefix)` if `recording.has_lrc`.
4. Calls `client.submit_component_analysis(...)` with all cached data.
5. Polls via `client.wait_for_completion(job_id)`.
6. Converts the result's `components` list to `list[SongComponent]` and calls `db_client.upsert_song_components(song_id, content_hash, components)`.
7. Returns the components.
...
After a successful full analysis (`analyze` command with `--wait`), optionally trigger component analysis since the allin1 sections are now freshly cached. Add a `--components` flag to the `analyze` command.
...
**Goal:** Re-run component extraction on existing analyzed songs using cached data (no re-download, no allin1 re-run).
...
| Neither | Skip gracefully | 0 s |

**Recommendation:** For batch backfill, filter to `analysis_status='completed'` first (fastest, highest coverage). Then process `lrc_status='completed'` songs. Skip songs with neither — left without components (transition logic falls back to global features).

### 4.3 Migration idempotency

All DDL uses `CREATE ... IF NOT EXISTS`, so running `sow-admin db init` adds the `song_components` table to existing databases without data loss. The `upsert_song_components` method uses DELETE+INSERT (not ON CONFLICT) so re-running backfill replaces stale component rows cleanly.
...
| `test_extract_components_fallback_to_lyrics` | No sections, lrc_content provided → lyrics path |
| `test_extract_components_fallback_to_r2` | No sections/lrc → downloads analysis.json from R2 (mocked) |
| `test_extract_components_skip` | No sections, no lrc, no R2 → returns `([], 'none')` |
...
Uses testcontainers (PostgreSQL) following existing test patterns. These tests are in the integration-excluded set (run with `--extra test`).
...
End-to-end: submit component analysis job → poll → verify result has components list. Uses mocked R2 (audio fixture), cached sections in request.
...
Existing recordings with analysis results but no component data work fine — `get_song_components()` returns `[]`, and transition logic falls back to global (whole-song) features. Component extraction is entirely optional and deferred.
...
Total for a typical backfilled song (tier 1): **~10–20 s**. For LRC-only songs (tier 2): **~10–20 s**. Songs with neither sections nor LRC are skipped.

### allin1 label reliability
allin1 section labels are ML-predicted and may mislabel a chorus as 'verse'. Fallback strategy:
1. If allin1 sections exist but contain NO 'chorus' label → fall through to lyrics-repetition (if LRC available).
2. If lyrics-repetition also fails (no repeated blocks) → return empty components (`source='none'`). The song uses global features.
3. The `confidence` field reflects the source: 0.9 (allin1), 0.7 (lyrics), 0.0 (none).

### Stems usage (optional future)
The `ComponentAnalysisOptions.use_stems` flag (default False) allows the feature extractor to use the drums stem for backbeat_strength and the vocals stem for groove_density — yielding more accurate per-component metrics. This is deferred for the first milestone; the plan's `compute_component_features` works on the full mix by default.
...
| `ops/analysis-service/tests/test_components.py` | Unit tests for identification algorithms and feature computation |
...
| `ops/admin-cli/tests/admin/test_audio_components.py` | CLI tests for `components` command and `show` enhancement |
...
| `ops/admin-cli/src/stream_of_worship/admin/services/analysis.py` | Add `submit_component_analysis()` to `AnalysisClient`; add `components`/`component_source` to `AnalysisResult`; update `_parse_job_response()` |
...
| `ops/analysis-service/src/sow_analysis/storage/r2.py` | Add `upload_component_result()` and `download_lrc_text()` methods |
