# Implementation Plan: Free-Only Patient Mode Flag (v4)

> **Version:** v4 — review fixes for `free-only-patient-mode-v3.md`.
> **Supersedes:** `free-only-patient-mode-v3.md`

## Review Summary (v3 → v4 Changes)

| Issue | Severity | Fix |
|---|---|---|
| `initialize()` DB write still resets progress to 0.0 (line 240) | Critical | Remove `progress=0.0` from both in-memory assignment (line 230) and DB `update_job()` call (line 240) |
| Startup warning says "fall back to Whisper" but handler FAILS the job | High | Fix warning text to say job will fail |
| Missing `set_qwen3_client()` setter + `_qwen3_client` attribute in wiring section | High | Add to §3 wiring and §5c |
| §3 line 210 says "module-level singleton" but mechanism is setter injection | Medium | Fix wording |
| `QuotaWaiter.__init__` initial event state unspecified | Medium | Specify event starts "set" (available) |
| Proposed LRC code doesn't restructure the `if` predicate (guard would be dead code) | High | Simplify `if` predicate to `request.options.use_qwen3_asr`; handle missing key via guard/client |
| `wait()` takes `job` param but doesn't use it | Low | Add periodic logging of waiting job IDs |
| Propose adding a config table row for free-only mode in startup logging | Low | Add to lifespan config table |
...
| **New QuotaWaiter module** | New file: `workers/quota_waiter.py` | New class — core abstraction | Medium (~160 lines) |
...
| **LRC worker** | `workers/lrc.py` | Accept optional `qwen3_client` kwarg in `generate_lrc_from_qwen3_asr`; use injected singleton instead of creating new client per call | Low |
...
| **Job queue / LRC pipeline** | `workers/queue.py` | Add QuotaWaiter + Qwen3Client setters/attributes; simplify `if` predicate in LRC handler; `lrc_source` init before retry loop; conditional wait-or-fallback in `except Qwen3AsrQuotaExhaustedError` handler (must be placed BEFORE `except Qwen3AsrError`); pass `stage_updater` + `mvsep_quota_waiter` to stem separation; guard against missing DashScope in free-only mode; heartbeat loop via `max_wait_seconds=60`; preserve progress in `initialize()` (both in-memory AND DB write) | Medium |
| **Application startup** | `main.py` | Import `Qwen3AsrClient` conditionally; create QuotaWaiter + Qwen3Client instances in lifespan, inject into queue via setters; stop QuotaWaiters in shutdown; startup validation warnings for missing keys in free-only mode; add free-only mode row to config table | Low |
| **Recovery / initialize** | `workers/queue.py` (`initialize()`) | Preserve `progress` from DB instead of resetting to `0.0` on restart — remove from BOTH in-memory assignment (line 230) AND DB `update_job()` call (line 240) | Trivial |
...
class QuotaWaiter:
    def __init__(self, name: str, probe_fn: Callable[[], bool], poll_interval: int):
        ...
        self._event.set()  # Start in "available" state — waiters resume immediately until mark_exhausted
        self._poller_task: Optional[asyncio.Task] = None
        self._waiting_jobs: set[str] = set()  # job IDs currently in wait()
    ...
    async def wait(
        ...
        The `job` parameter is used for periodic logging: every 30s, logs the
        count of waiting jobs and a sample of job IDs, so operators can see what's
        blocked.
...
### Periodic Logging in `wait()`

The `job` parameter is used for periodic logging. Every 30 seconds (checked via `time.monotonic()`), `wait()` logs a summary:
...
if now - self._last_log_time >= 30.0:
    ...
    logger.info(
        ...
        self._name, len(self._waiting_jobs), sample,
    )
```

On entry, `wait()` adds `job.id` to `self._waiting_jobs`; on exit, removes it.
...
**New conditional import** (alongside existing `MvsepClient`, `AudioSeparatorWrapper`, `ForcedAlignerWrapper`):
...
- Create `Qwen3AsrClient` singleton if `Qwen3AsrClient is not None` and `settings.SOW_DASHSCOPE_API_KEY` is truthy — inject via `job_queue.set_qwen3_client(qwen3_client)` (new setter). This ensures `lrc.py`'s `generate_lrc_from_qwen3_asr` and the `QuotaWaiter`'s `probe_fn` reference the **same instance**, so `_quota_exhausted` flag changes are visible to both.
- Create `mvsep_quota_waiter` with `probe_fn=mvsep_client.is_available` (if MVSEP is configured)
- Create `qwen3_quota_waiter` with `probe_fn=qwen3_client.is_available` (if Qwen3 client is configured)
- Inject both into `job_queue` via new setter method `set_quota_waiters(mvsep=..., qwen3=...)`
...
- **Add free-only mode row to the startup config table**:
...
- `await mvsep_quota_waiter.stop()` / `await qwen3_quota_waiter.stop()` (guard with `if mvsep_quota_waiter is not None:` etc.)
...
def set_qwen3_client(self, qwen3_client):
...
**2. Add `stage_updater` and `mvsep_quota_waiter` parameters to `_separate_with_mvsep_fallback()`** (`stem_separation.py:209`). Current signature has 6 params; new signature adds 2:
...
The existing inner-stage transitions (`"fallback_local"`, `"fallback_local_stage2"`, individual Stage 1/Stage 2 events) can remain using `_set_job_stage()` (in-memory only). Only the **waiting-stage transitions** need the `stage_updater` callback, since those are the ones that persist for hours and need DB visibility. For consistency, the implementation MAY route all inner-stage transitions through `stage_updater` when available, but this is optionals.

#### Service Restart Preserves Progress (FIXED: both in-memory AND DB)

The `initialize()` method at `queue.py:217-264` currently resets `job.progress = 0.0` for all interrupted jobs **twice**: once in-memory (line 230) and once in the DB write (line 240). Both must be fixed:

```python
# Before (current):
for job in interrupted:
    job.status = JobStatus.QUEUED
    job.progress = 0.0            # ← line 230: remove this
    job.stage = "requeued"
    job.updated_at = datetime.now(timezone.utc)
    ...
    try:
        await self.job_store.update_job(
            job.id, status="queued", progress=0.0, stage="requeued"  # ← line 240: remove progress=0.0
        )
...
Jobs that were interrupted before reaching the quota-wait stage (e.g., during download) will still have `progress=0.0` from their initial state (deserialized from DB). The fix only preserves progress for jobs that had already progressed.
...
**Singleton lifecycle**: Because `generate_lrc_from_qwen3_asr` in `lrc.py` currently creates a new `Qwen3AsrClient` per call (at `lrc.py:818-823`), the QuotaWaiter's `probe_fn` must reference the **same instance**. The solution is to make `Qwen3AsrClient` a singleton created in `main.py` lifespan, injected into `JobQueue` via `set_qwen3_client()`, and passed as an optional `qwen3_client` kwarg to `generate_lrc_from_qwen3_asr`. When not provided (backward compatible), fall back to creating a new client.

**Changes to `lrc.py`**:

In `generate_lrc_from_qwen3_asr` (currently at `lrc.py:793-803`), add optional `qwen3_client` kwarg:
...
async def generate_lrc_from_qwen3_asr(
    ...
    qwen3_client: Optional["Qwen3AsrClient"] = None,  # NEW
...
In `main.py` lifespan (lines 75-84, alongside MVSEP client creation):
...
### 5d. `queue.py` LRC Handler Restructure (FIXED)

**Critical context**: The current Qwen3 ASR block at `queue.py:1112-1168` is gated by an `if` predicate (lines 1112-1117) that checks `request.options.use_qwen3_asr AND settings.SOW_DASHSCOPE_API_KEY AND generate_lrc_from_qwen3_asr is not None AND build_qwen3_asr_cache_key is not None`, with an `else` branch (lines 1166-1168) that logs "Qwen3 ASR disabled" and falls to Whisper.

**Problem with v3 spec**: The v3 spec proposed a guard at the top of the `while True` loop checking `SOW_FREE_ONLY_MODE and use_qwen3_asr and not SOW_DASHSCOPE_API_KEY`, but this guard would be **dead code** — the `if` predicate already requires `SOW_DASHSCOPE_API_KEY` to be truthy, so the code would never enter the block when the key is missing.

**Fix in v4**: Simplify the `if` predicate to just `request.options.use_qwen3_asr` (drop the `SOW_DASHSCOPE_API_KEY`, `generate_lrc_from_qwen3_asr is not None`, and `build_qwen3_asr_cache_key is not None` checks). The missing-key case is then handled by:
- Free-only mode: the guard raises `LrcWorkerError` (fails the job)
- Non-free-only mode: `generate_lrc_from_qwen3_asr` internally calls `Qwen3AsrClient.transcribe()`, which raises `Qwen3AsrNonRetriableError("SOW_DASHSCOPE_API_KEY is not configured")` — a subclass of `Qwen3AsrError` — caught by `except Qwen3AsrError` → Whisper fallback. If the import failed (function is None), `TypeError: 'NoneType' not callable` is caught by `except Exception` → Whisper fallback.

**Note**: If the lrc module import fails entirely, `Qwen3AsrError` is aliased to `Exception` (line 104), and `generate_lrc_from_qwen3_asr` is `None` (line 108). Calling `None(...)` raises `TypeError`, caught by the `except Exception` handler → Whisper fallback. This is acceptable per design decision — assume imports succeed.

```python
# --- Restructured Qwen3 ASR block (replaces lines 1112-1168) ---
...
**Import requirement**: `queue.py` must also import `Qwen3AsrQuotaExhaustedError` from `qwen3_asr_client`. The existing import at line 101 currently only imports `Qwen3AsrError`. Update:

```python
from ..services.qwen3_asr_client import Qwen3AsrError, Qwen3AsrQuotaExhaustedError
```

And the fallback in the `except ImportError` block (line 102-110) must add:
```python
Qwen3AsrQuotaExhaustedError = Exception  # alias so except clauses work even if import fails
```
...
### 5e. Pausing the MVSEP Time Budget
...
| **Recovery** | `initialize()` no longer resets `progress` to `0.0` (neither in-memory nor in DB). This is safe for non-free-only-mode jobs because their progress was already `0.0` at the point of interruption (they weren't in a quota-wait state). |
| **`if` predicate simplification** | The `if request.options.use_qwen3_asr:` predicate (was `if use_qwen3_asr and DASHSCOPE_KEY and ... is not None`) now enters the block even when keys are missing. In non-free-only mode, this produces a `Qwen3AsrNonRetriableError` from the client, caught by `except Qwen3AsrError` → Whisper. Functionally identical to the old `else` branch. |
...
Existing recovery at `queue.py:217-264` handles this: `PROCESSING` jobs are reset to `QUEUED` with `stage="requeued"`. Progress is **preserved from DB** (no longer reset to `0.0`, both in-memory and DB write). On restart:
...
### LRC handler `if` predicate change in non-free-only mode
When `SOW_DASHSCOPE_API_KEY` is empty and `use_qwen3_asr=True` in non-free-only mode, the restructured code enters the `if` block (predicate is just `use_qwen3_asr`). The guard at the top (`if SOW_FREE_ONLY_MODE and not key: raise`) does NOT fire (free-only mode is off). The code proceeds to call `generate_lrc_from_qwen3_asr(...)`, which internally creates/reuses a `Qwen3AsrClient` whose `transcribe()` raises `Qwen3AsrNonRetriableError("SOW_DASHSCOPE_API_KEY is not configured")`. This is a subclass of `Qwen3AsrError`, caught by `except Qwen3AsrError` → Whisper fallback. Functionally identical to the old behavior where the `else` branch logged "Qwen3 ASR disabled" and went to Whisper. The only visible difference is the warning text.
...
| `initial_event_state_is_set` | New QuotaWaiter — wait() returns True immediately before mark_exhausted |
| `periodic_logging_emits_waiting_count` | 2 jobs waiting, advance time 30s → log line with count=2 and sample IDs |
...
| `lrc_missing_key_non_free_only_falls_back_to_whisper` | `SOW_FREE_ONLY_MODE=False`, no DashScope key, `use_qwen3_asr=True` → client raises NonRetriableError → Whisper fallback |
| `lrc_use_qwen3_false_uses_whisper` | `use_qwen3_asr=False` → goes straight to Whisper, never enters Qwen3 block |
...
| `progress_preserved_on_restart` | Job in quota-wait with progress=0.4 → service restart → job requeued with progress=0.4 (not 0.0) — verifies BOTH in-memory and DB preservation |
| `initialize_db_write_preserves_progress` | Verify `update_job()` call in `initialize()` does NOT pass `progress=0.0` |
...
|---|---|---|
| `workers/quota_waiter.py` | **New** | Medium — QuotaWaiter class (~160 lines) |
| `config.py` | Add 2 fields | Trivial |
| `.env.example` | Document 2 vars | Trivial |
| `services/qwen3_asr_client.py` | Add `_quota_exhausted` flag, `is_available`, `is_quota_exhausted`, `_check_quota_reset`, `Qwen3AsrQuotaExhaustedError`; modify `_raise_for_response` for 429; modify `_with_retries` to catch/set/re-raise; modify `transcribe` fast-fail | Medium |
| `workers/lrc.py` | Accept optional `qwen3_client` kwarg in `generate_lrc_from_qwen3_asr`; use injected singleton | Low |
| `services/mvsep_client.py` | Add `is_quota_exhausted` property | Trivial |
| `workers/stem_separation.py` | Add `stage_updater` + `mvsep_quota_waiter` parameters; conditional wait at 3 decision points; time-budget pause for both `_time_remaining()` and `_stage2_time_remaining()`; fail on permanent unavailability in free-only mode | Medium |
| `workers/queue.py` | Add QuotaWaiter + Qwen3Client setters/attributes; import `Qwen3AsrQuotaExhaustedError`; simplify `if` predicate in LRC handler; `lrc_source` init before retry loop; LRC wait-retry loop with heartbeat and correct except ordering; guard against missing DashScope in free-only mode; pass `stage_updater` + `mvsep_quota_waiter` to stem separation; preserve progress in `initialize()` (both in-memory AND DB write) | Medium |
| `main.py` | Conditional import `Qwen3AsrClient`; create QuotaWaiter + Qwen3Client instances in lifespan, inject into queue via setters; stop QuotaWaiters in shutdown; startup validation warnings for missing keys in free-only mode; add free-only mode row to config table | Low |
...
| `Qwen3AsrClient.__init__` | `services/qwen3_asr_client.py:92-102` |
...
| `_set_job_stage()` (in-memory only) | `workers/stem_separation.py:201-206` |
| `process_stem_separation()` | `workers/stem_separation.py:318-536` |
| `generate_lrc_from_qwen3_asr` | `workers/lrc.py:793-803` (creates new client at `lrc.py:818-823`) |
| `_process_lrc_job()` | `workers/queue.py:870` |
| Qwen3 ASR try/except block | `workers/queue.py:1112-1168` (being restructured) |
| `lrc_source` initialization | `workers/queue.py:1069` |
| Whisper fallback block | `workers/queue.py:1170-1213` |
| `_update_stage()` (persists to DB) | `workers/queue.py:766-782` |
| `JobQueue.initialize()` (recovery) — in-memory reset | `workers/queue.py:230` |
| `JobQueue.initialize()` (recovery) — DB reset | `workers/queue.py:239-240` |
...
| `JobQueue.set_separator_wrapper()` | `workers/queue.py:193-199` |
| `JobQueue.set_mvsep_client()` | `workers/queue.py:201-207` |
| `queue.py` Qwen3 import | `workers/queue.py:101` |
| `queue.py` import fallback alias | `workers/queue.py:102-110` |
| `optional_semaphore()` | `workers/queue.py:30-40` |
| `process_jobs()` (main loop) | `workers/queue.py:350-379` |
| `lifespan` (startup/shutdown) | `main.py:50-193` |
| `main.py` conditional imports | `main.py:22-35` |
| `main.py` module-level globals | `main.py:37-47` |
| `main.py` startup config table | `main.py:110-160` |
| `main.py` shutdown logic | `main.py:170-192` |
...
## Appendix B: Changes from v3 (Review Fixes)

| Issue | Severity | Section(s) | Fix Applied in v4 |
|---|---|---|---|
| `initialize()` DB write still resets progress to 0.0 (line 240) | Critical | 4, 7 | Remove `progress=0.0` from BOTH in-memory assignment (line 230) AND `update_job()` DB call (line 240) |
| Startup warning says "fall back to Whisper" but handler FAILS the job | High | 3 | Fixed warning text to say "will FAIL" / "permanently unavailable" |
| Missing `set_qwen3_client()` setter + `_qwen3_client` attribute in wiring | High | 3, 5c | Added to §3 wiring (new instance attribute + new setter) and §5c (called in lifespan) |
| §3 line 210 says "module-level singleton" but mechanism is setter injection | Medium | 3 | Fixed wording: singleton created in lifespan, injected via `set_qwen3_client()` setter |
| `QuotaWaiter.__init__` initial event state unspecified | Medium | 3 | Specified: `self._event.set()` in `__init__` — starts in "available" state |
| Proposed LRC code: guard would be dead code (if predicate requires key) | High | 5b | Simplified `if` predicate to `request.options.use_qwen3_asr` only; dropped `SOW_DASHSCOPE_API_KEY`, `generate_lrc_from_qwen3_asr is not None`, `build_qwen3_asr_cache_key is not None` checks. Missing key handled by guard (free-only) or client NonRetriableError (non-free-only). Documented in §7 edge cases. |
| `wait()` takes `job` param but doesn't use it | Low | 3 | Added periodic logging (every 30s) of waiting job count and sample of job IDs |
| Missing free-only mode config table row in startup | Low | 3 | Added two rows to startup config table: `enabled` and `poll_interval_seconds` |
| Missing `Qwen3AsrQuotaExhaustedError` import in `queue.py` | Medium | 5b | Added to import at line 101 and fallback alias in except block |
| Missing `Qwen3AsrClient` import in `main.py` | Medium | 3, 5c | Added conditional import block (try/except ImportError) alongside existing imports |
