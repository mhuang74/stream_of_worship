...
| `enrich_pool` calls `classify_embedding_themes` (numpy cosine in Python) | `enrich_pool` calls `_normalise_cosine_scores` directly on pre-computed SQL scores; `classify_embedding_themes` and `load_theme_anchors` no longer called |
...
- `numpy` is still needed in `constructor` for `_normalise_cosine_scores` and phase inference, but NOT for vector cosine (moved to SQL).
