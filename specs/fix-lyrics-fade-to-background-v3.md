# Fix Lyrics Fade To Background V3
...
- Add a private helper on `FrameRenderer` for flat-background RGB compositing:
  ...
  - Output: RGB tuple computed per channel as
    `int(background + (foreground - background) * alpha / 255)`.
    Use `int()` truncation (not `round()`) to preserve the existing color rounding behavior.
...
- Make the rendered-pixel assertion **channel-wise** and scoped to pixels changed from the
  background, using a template where text/highlight channels are **strictly brighter** than the
  background in every channel (e.g., `gradient_warm` or `gradient_blue`).
- Assert that for every changed pixel, each channel value is `>=` the corresponding background
  channel value. This ensures the composited fade never dips below the background color.
- Run:
