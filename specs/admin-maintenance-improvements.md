# Admin CLI Maintenance Commands Improvements
...
| MB in JSON | Yes — replace `total_bytes` with `total_mb` (integer) in both table and JSON |
| last_modified format | Space-separated datetime truncated to seconds: `YYYY-MM-DD HH:MM:SS` |
...
| repair-songsets `--all` conflict | `--all` = list all; `--all --confirm` = repair all. `--confirm` determines list-vs-repair action. |
...
Applies display transformations to all rows before printing. Transforms:
- `total_bytes` → renamed to `total_mb`, value = `_bytes_to_mb(total_bytes)`
- `r2_bytes` → renamed to `r2_mb`, value = `_bytes_to_mb(r2_bytes)` *(consistency: same unit convention for all byte-valued fields)*
- `last_modified` → `_format_datetime(last_modified)`
- `created_at` → `_format_datetime(created_at)`

```python
_BYTE_FIELDS = {"total_bytes": "total_mb", "r2_bytes": "r2_mb"}
_DATETIME_FIELDS = {"last_modified", "created_at"}
...
Call `_transform_rows(rows)` at the top, before branching on format:
...
**Impact**: All commands that output `total_bytes`, `r2_bytes`, `last_modified`, or `created_at` fields get automatic formatting. This includes `list-r2-waste`, `purge-r2-waste`, `list-soft-deletes --with-r2`, and the new `repair-songsets` listing.

**Note**: The `--format ids` path in `list-soft-deletes` returns before `_print_manifest`, so it is unaffected.
...
### B.1 `list-soft-deletes` (line 107-158)

**Signature changes:**
- `limit: Optional[int] = typer.Option(None, "--limit", min=1)` → `typer.Option(20, "--limit", min=1)`
...
```
Pass `effective_limit` to `list_soft_deleted_songs_with_counts(limit=effective_limit)` and `list_soft_deleted_recordings_with_counts(limit=effective_limit)`.

**Note**: `--entity all` (which entities to list) and `--all` (no limit) are independent options. No conflict, but users should be aware of the distinction.

### B.2 `list-r2-waste` (line 400-410)
...
### B.3 `diagnose-render-failures` (line 353-372)
...
effective_limit = None if all_ else limit
```
Pass `effective_limit` to `find_failed_render_jobs(limit=effective_limit)`.

### B.4 Commands NOT changed

- `purge-soft-deletes` — keeps existing `--limit` (default `None`) and existing `--all` (process all matching). No change.
- `restore-soft-deletes` — no `--limit` flag, no change.
- `purge-r2-waste` — no `--limit` flag, no change.
...
### C.2 Modify `_orphan_r2_prefixes` (line 375-397)
...
- `list_r2_waste`: call `_orphan_r2_prefixes(db_client, r2_client, config.r2_waste_blacklist)` (no limit arg), then sort and limit in the command body (see B.2).
- `purge_r2_waste` (line 428): call `_orphan_r2_prefixes(db_client, r2_client, config.r2_waste_blacklist)` (remove the `None` limit arg).
...
**SQL notes:**
- The `EXISTS` subquery uses the same stale condition as `find_stale_songset_items` (line 1484-1485): `recording_hash_prefix IS NOT NULL AND (r.hash_prefix IS NULL OR r.deleted_at IS NOT NULL)`.
...
- JOIN to `"user"` table (quoted, since `user` is a SQL reserved word) for email. The table is created by Better Auth (webapp/src/db/schema.ts:110).
- `LEFT JOIN "user"` so songsets are still returned even if the user row is missing (email defaults to `""`).

### D.2 Modify `repair-songsets` command (line 316-350)
...
| `repair-songsets --all` | List | List all songsets needing repair (no limit) |
...
| `repair-songsets --confirm` | List | Falls to list mode (no target, not --all --confirm). `--confirm` is ignored. |

---

## Change E: Imports

Add `datetime` import to `maintenance.py` (currently only imports from `pathlib.Path` and `typing.Optional`):
...
2. **`test_maintenance_list_soft_deletes_ids`** (line 190-203):
   - `FakeMaintenanceDb.list_soft_deleted_recordings_with_counts` returns 1 recording. Default limit 20 is fine (1 < 20). Test should still pass. Verify.
...
9. **`test_diagnose_render_failures_defaults_to_limit_20`**:
   - Mock DB to return 25 failed jobs. Verify only 20 are shown. Verify `--all` shows all 25.

10. **`test_repair_songsets_no_args_lists_songsets_needing_repair`**:
...
6. **`deleted_at` in `list-soft-deletes`**: Currently ISO string. NOT in `_DATETIME_FIELDS` set, so NOT reformatted. Only `last_modified` and `created_at` are reformatted. *(If desired, add `deleted_at` to `_DATETIME_FIELDS` for full consistency — not included per user's specific request for `last_modified` only.)*

7. **`"user"` table access**: The admin DB client connects to the same Postgres as the webapp. The `"user"` table is created by Better Auth (Drizzle). The `LEFT JOIN "user" u ON u.id = s.user_id` should work. If the table doesn't exist (e.g., admin-only DB), the query will fail. Consider wrapping in a try/except for `UndefinedTable` like `find_active_render_jobs_for_songsets` does. **Recommendation**: Add `UndefinedTable` catch returning empty list, since the admin DB might not have Better Auth tables in all environments.

8. **`repair-songsets --confirm` (no target)**: Falls to list mode. `--confirm` is silently ignored. This is a minor UX issue but acceptable — `--confirm` without a target doesn't make sense for repair.

9. **Column ordering in table output**: `_print_manifest` sorts columns alphabetically. For repair-songsets listing, columns will appear as: `created_at`, `name`, `song_count`, `songset_id`, `user_email`. This is fine.
