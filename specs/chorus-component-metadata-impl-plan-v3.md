...
# Implementation Plan: Chorus-Based Song Component Metadata (v3)

> **Revision Notes:** This supersedes `specs/chorus-component-metadata-impl-plan-v2.md` (v2).
> Key changes from v2:
>
> 1. **Fixed unique constraint violation.** The previous schema's
>    `idx_song_components_unique` was on `(song_id, component_type, occurrence_index)`,
>    which made it impossible to persist two role-differentiated rows for a single-chorus
>    song (one `role='entry'`, one `role='exit'`, both `occurrence_index=1`). The new v3
>    unique index includes `role`: `(song_id, component_type, occurrence_index, role)`.
> 2. **Lyrics-repetition disambiguation strengthened.** The v2 algorithm picked the
>    highest-scoring repeated block as THE chorus. Worship songs frequently repeat
>    verses (stanza 1/2/3 with identical lyrics) more than the chorus, causing
>    misidentification. v3 uses multi-cue weighting (repetition × position × length ×
>    lyrical-content cues). See Phase 1.1, Strategy 2.
> 3. **Worker re-checks R2 cache (defense in depth).** v2 had a duplicated cache-check
>    contract (admin CLI + worker). v3 keeps BOTH layers: admin CLI short-circuits for
>    fast user feedback; the worker independently re-checks R2 cache to handle
>    programmatic submissions and job replays. The cache check lives inside
>    `extract_components()` itself.
> 4. **`components.json` carries a `schema_version` field.** v2's cache payload had no
>    version marker, so future algorithm/field changes would silently parse stale R2
>    objects. v3 adds `"schema_version": 1`; incompatible versions are treated as cache
>    misses and recomputed.
> 5. **Tier-2 (LRC-only) auto-runs fast_analyze inline.** v2 silently fell back to
>    librosa-on-slice only when beats were missing, producing unsnapped boundaries and
>    unreliable per-component BPM. v3 component worker, when `beats`/`downbeats` are
>    absent from the request, first runs the equivalent of the fast_analyze pipeline
>    inline (reusing `analyzer.analyze_audio_fast` from
>    `ops/analysis-service/src/sow_analysis/workers/analyzer.py:451`) on the same audio
>    to populate `beats`/`downbeats`/`tempo_bpm`. Result is cached via
>    `CacheManager.save_fast_analyze_result` so subsequent component jobs reuse it.
> 6. **Dedicated `_component_semaphore`.** v2 reused `_fast_analyze_semaphore`, risking
>    backfill-induced starvation of live `fast_analyze` traffic. v3 adds
>    `self._component_semaphore = asyncio.Semaphore(settings.SOW_COMPONENT_MAX_CONCURRENT)`
>    (new setting, same cgroup-aware default pattern as
>    `SOW_FAST_ANALYZE_MAX_CONCURRENT`).
> 7. **Reuses existing `workers/lrc_parser.py`.** v2 incorrectly claimed the analysis
>    service lacked an LRC parser and proposed porting a new one. The codebase already
>    ships `ops/analysis-service/src/sow_analysis/workers/lrc_parser.py` (vendored copy
>    of admin-cli's `parse_lrc`, exposing `LRCLine{time_seconds, text, raw_timestamp}`
>    and `LRCFile`). v3 imports `parse_lrc` from there; no new parser is written.
> 8. **Method naming kept as-is.** Confirmed by review that the analysis-service
>    `R2Client` and admin-cli `R2Client` are separate classes in separate packages.
>    The analysis-service exposes `download_lrc_text(hash_prefix)`; the admin-cli
>    exposes `download_lrc_content(hash_prefix)`. v3 documents this as intentional.
> 9. **Audio URL freshness.** Confirmed by review: the request's `audio_url` follows
>    the same convention as existing analyze jobs (long-lived R2/S3 URLs, not pre-signed
>    with short expiry). No version bump needed for v1 of this feature.
> 10. **Cache file naming follows existing convention.** Local cache file use
>     `{hash_prefix}_components.json` (matches `{hash_prefix}_fast.json`,
>     `{hash_prefix}_lrc.json`); R2 object key uses `{hash_prefix}/components.json`
>     (matches `{hash_prefix}/analysis.json`, `{hash_prefix}/lyrics.lrc`).
...
The `allin1` library runs Demucs source separation + a structural segmentation neural
network. On a laptop CPU this costs **30–60 minutes per song**. The prompt observes that
"chorus can be easily identified via lyrics repetition," suggesting we may not need
allin1 at all.
...
1. **allin1 sections are already cached** for every song with `analysis_status='completed'`.
   ...
   `[{"label","start","end"}]`, and the full `analysis.json` (including `beats`,
   `downbeats`, `sections`) is mirrored to R2 at `{hash_prefix}/analysis.json`. **Extracting
   components from already-analyzed songs costs nothing** — the expensive Demucs run already
   happened.

2. **LRC is optional** (`lrc_status` tracks `pending`/`processing`/`completed`/`failed`),
   but when present it is a timestamped line list (`LRCLine{time_seconds, text}`).
   Choruses repeat verbatim or near-verbatim across occurrences, making them detectable via
   repeated-line-group clustering in **<1 second** per song.

3. **Per-component audio features** (groove_density, backbeat_strength, energy_level,
   per-segment BPM/key) require librosa on the audio slice regardless of identification
   method. This is fast (~5–15 s/song) and does NOT need allin1 or Demucs.

4. **No existing repetition/chorus detection** exists — only a stub `_detect_sections()`
   returning `unknown`.

5. **Existing LRC parser available.** `ops/analysis-service/src/sow_analysis/workers/lrc_parser.py`
   already exposes `parse_lrc(content: str) -> LRCFile` with `LRCLine{time_seconds, text,
   raw_timestamp}` (vendored byte-identical copy of admin-cli's parser). Reuse directly —
   do NOT write a new parser.

6. **Existing fast_analyze function available.** `ops/analysis-service/src/sow_analysis/workers/analyzer.py:451`
   defines `async def analyze_audio_fast(...)` which produces `beats`/`downbeats`/
   `tempo_bpm` via librosa in seconds. Reuse inline when tier-2 lacks beats (see Phase 2.3).
...
| 2 | Lyrics-repetition from LRC lines | **<1 s (+ ~3 s for inline fast_analyze if beats missing)** | Songs with `lrc_status='completed'` but no full analysis |
| — | Skip gracefully | **0 s** | Songs where none of the above succeed |

**Why not lyrics-only?** It discards reliable, already-computed allin1 structural labels
for the completed-analysis catalog, and fails entirely for the (common) case where LRC is
missing. The hybrid approach maximizes coverage at minimum cost.

**Why not allin1-only?** Re-running allin1 is prohibitively expensive (30–60 min/song),
and many catalog songs without full analysis already have LRC. Tier-2 covers them.

**Benefit of hybrid:** Backfill of the entire completed-analysis catalog is instant
(tier 1). New songs with LRC get components in seconds (tier 2; inline fast_analyze adds
a few seconds when cached allin1 beats aren't available). Songs with neither sections
nor LRC are skipped gracefully — the caller is responsible for generating LRC if needed.

### Where Component Extraction Runs

Per-component feature computation (librosa) requires the **audio slice**. Per the
architectural separation rule (Admin CLI never imports ML libraries), all identification
AND feature computation happens in the **Analysis Service** via a new `COMPONENT_ANALYSIS`
job type. The Admin CLI submits the job (passing cached `sections`/`lrc_content`/
`beats`/`downbeats` to avoid re-computation), polls for completion, and persists the
returned component rows to `song_components`.
...
Re-export the new constants and insert them into `ALL_SCHEMA_STATEMENTS` in the catalog
section (after `CREATE_RECORDINGS_UPDATE_TRIGGER`):
...
class SongComponent:
    ...
    Attributes:
        ...
        role: 'entry' | 'exit' | 'loop_target' | 'entry_exit' | 'none'.
            v3: 'entry_exit' is reserved for persistence-time convenience if a single
            row is preferred over two rows for single-chorus songs. The default
            persistence path emits two rows ('entry' + 'exit') for single-chorus songs,
            enabled by the v3 unique index that includes role.
...
def upsert_song_components(
    ...
    v3: the unique index includes `role`, so callers MAY persist two rows with
    the same (song_id, component_type, occurrence_index) and differing role
    (e.g. entry+exit for a single-chorus song).
    """
...
psql "$DATABASE_URL" -c "
  -- Verify unique index includes role
  SELECT indexdef FROM pg_indexes WHERE indexname = 'idx_song_components_unique';
"
```
...
allin1-section labels and multi-cue lyrics-repetition clustering) and per-component audio
feature computation.
...
def identify_from_allin1_sections(
    ...
    - v3: If only 1 chorus → persist as TWO ComponentInstance rows
      ...
      with identical start_time/end_time. The v3 unique index
      (song_id, component_type, occurrence_index, role) permits this.
...
- Multiple consecutive 'verse' sections before the first chorus → take the last verse
  section before the chorus (closest to it).
- 'prechorus' labels → ignored for this milestone (future rows, no schema migration
  needed).

#### Identification Strategy 2: `identify_from_lyrics_repetition` (Multi-cue v3)
...
def identify_from_lyrics_repetition(
    ...
    v3 — Multi-cue weighting (replaces v2's pure `repeat_count * w` scoring,
    which misidentified repeated verses as choruses).
    ...
    1. Parse LRC via the EXISTING workers/lrc_parser.py:
            ...
            lines = lrc_file.lines  # list[LRCLine(time_seconds, text, raw_timestamp)]
    ...
    4. v3 multi-cue SCORING — for each candidate chorus (repeat_count >= 2,
       window size w, occurrence times t[0], t[1], ..., t[k-1]):
          repetition_score = min(repeat_count, 4) * w
             # Capped so a 4x stanza doesn't drown out a 2x chorus.
          position_weight = 1.0 if t[0] > 0.1 * total_duration else 0.4
             # Choruses rarely open a song; verses often do.
          length_weight = 1.0 if 4 <= w <= 8 else 0.6
             # Worship choruses are typically 4-8 lines.
          content_weight = 1.4 if any(kw in joined_text.lower() for kw in
              ('chorus', '赞美', '敬拜', '主', '宝座', '圣', 'sing', 'hallelujah',
               '哈利路亚', '羔羊', '王', '恩典'))
             else 1.0
             # Soft lyrical-content nudge. Does not override repetition but
             # breaks ties toward chorus-typical vocabulary.
          final_score = repetition_score * position_weight * length_weight
                        * content_weight
       Tiebreak: highest repetition_score first, then earliest t[0].

    5. Best candidate = chorus. Its occurrence positions give N chorus instances:
       - occurrence 1 → role='entry'
       - occurrence N → role='exit'
       - v3: if N==1 → persist as TWO ComponentInstance rows
         (occurrence_index=1, role='entry' AND occurrence_index=1, role='exit'),
         identical start/end (mirrors allin1 Strategy 1 single-chorus handling).
       start_time = first_line.time_seconds of each occurrence.
       end_time = next line's time_seconds after the block (or
          last_line.time_seconds + estimated duration).

    6. Verse (loop_target): the lines immediately before the first chorus
       occurrence. Walk backward from the chorus start until a time-gap >
       threshold (e.g., 3s) or the song start. start_time = first verse line
       time, end_time = chorus start. occurrence_index=1.

    7. Snap start_time/end_time to nearest beat IF beats provided (see Phase 2.3
       for tier-2 fast_analyze fallback when not provided).
    """
```

**Fuzzy matching note:** `rapidfuzz` is already a dependency (used in
`services/canonical_snap.py`). Import `from rapidfuzz import fuzz`.

**LRC parser note:** `ops/analysis-service/src/sow_analysis/workers/lrc_parser.py`
already exists (vendored from admin-cli). Import `parse_lrc` from there. Do NOT write
a new parser.
...
async def extract_components(
    ...
    respectively, even when the admin CLI already short-circuited. This protects
    programmatic submissions and job replays.
    ...
    1. UNLESS force=True: check local cache → R2 cache. If a cached
       components.json exists AND its `schema_version` matches the current
       COMPONENT_SCHEMA_VERSION (=1), return the cached payload.
    ...
    3. Elif lrc_content provided & non-empty:
       a. If beats/downbeats NOT provided → run analyze_audio_fast() inline
          (see Phase 2.3) to obtain beats/downbeats/tempo_bpm. Save result via
          CacheManager.save_fast_analyze_result for reuse.
       b. identify_from_lyrics_repetition(lrc_content, beats, downbeats, ...)
    4. If all fail → ([], 'none').

    After identification, ALWAYS load audio (if not already loaded) and compute
    per-component features via compute_component_features().

    Finally, persist the result to local cache AND R2 with `schema_version=1`.
    """
```
...
def compute_component_features(
    ...
    - confidence: heuristic — 0.9 if from allin1 labels, 0.7 if from lyrics.
      ...
      v3: if lyrics path was used AND beats were missing (no fast_analyze
      fallback ran, e.g., the audio load failed), reduce confidence to 0.5.
    """
```
...
queue processing, cache strategy, and dedicated concurrency semaphore.
...
### 2.2 `ops/analysis-service/src/sow_analysis/config.py`

Add a new setting following the same cgroup-aware default pattern as
`SOW_FAST_ANALYZE_MAX_CONCURRENT`:

```python
# Maximum concurrent component_analysis jobs. 0 = auto (cgroup-aware on Linux).
...
def _validate_component_max_concurrent(cls, v: int) -> int:
    if v < 0:
        raise ValueError("SOW_COMPONENT_MAX_CONCURRENT must be >= 0 (0 = auto)")
    return v
```

### 2.3 `ops/analysis-service/src/sow_analysis/routes/jobs.py`
...
### 2.4 `ops/analysis-service/src/sow_analysis/workers/queue.py`
...
try:
    ...
    from .analyzer import analyze_audio_fast  # for tier-2 inline beat fallback
except ImportError:
...
**v3 — Dedicated semaphore** (do NOT reuse `_fast_analyze_semaphore`):
...
async def _process_component_analysis_job(self, job: Job) -> None:
    ...
    2. Download audio from R2 to temp dir (audio_url from the request; this
       follows the same convention as analyze jobs — long-lived R2/S3 URLs,
       not short-lived pre-signed URLs, so queue staleness is not an issue).
    3. Check CacheManager.get_component_result(content_hash) — if cached AND
       schema_version == COMPONENT_SCHEMA_VERSION AND not force, short-circuit
       and return cached payload.
    4. Check R2 for {hash_prefix}/components.json — same schema_version check.
    5. If neither cached: call extract_components() with cached sections /
       lrc_content from the request.
       - INSIDE extract_components (tier-2 path): if beats are missing, call
         analyze_audio_fast(audio_path, content_hash, cache_manager, r2_client)
         inline (reuses ops/analysis-service/src/sow_analysis/workers/analyzer.py:451).
         The result is saved via cache_manager.save_fast_analyze_result so
         subsequent component jobs and any fast_analyze jobs reuse it. If that
         inline call also fails, fall back to librosa-on-slice only and lower
         component confidence to 0.5.
    6. Convert list[ComponentInstance] → list[ComponentResult].
    7. Upload component results to R2 as {hash_prefix}/components.json
       (with schema_version=1).
    8. Save to local cache via cache_manager.save_component_result().
    9. Build JobResult(components=..., component_source=...).
    10. Set status COMPLETED, stage 'complete'.
    11. Persist to JobStore.
    """
```
...
```python
COMPONENT_SCHEMA_VERSION = 1  # bump when components.json shape changes
...
**v3 JSON schema for `components.json`** (note the new `schema_version` field):
...
### 2.6 `ops/analysis-service/src/sow_analysis/storage/r2.py`
...
async def upload_component_result(self, hash_prefix: str, result: dict) -> str:
    ...
    The `result` dict MUST include `schema_version`. Returns the R2 object key.
    """
...
async def download_component_result(self, hash_prefix: str) -> Optional[dict]:
    ...
    OR if schema_version mismatches the current COMPONENT_SCHEMA_VERSION).

    Checks existence first, downloads to a temp file, parses JSON, cleans up.
    """
    # Uses check_exists + download_to_temp + json.loads + schema_version check
...
async def download_lrc_text(self, hash_prefix: str) -> Optional[str]:
    ...
    the admin-cli R2Client's `download_lrc_content` (Phase 3.2). The two
    R2Client classes live in separate packages and have different conventions;
    the name divergence is documented and accepted.
    """
    # Uses check_exists on {hash_prefix}/lyrics.lrc + download_to_temp + read_text
```
...
**Goal:** Submit component analysis jobs, persist results to `song_components`, display
component metadata in the CLI.
...
def submit_component_analysis(
    ...
    AND its schema_version is current, this method returns the cached result
    directly (unless force=True).
    """
```
...
def get_cached_component_result(self, hash_prefix: str) -> Optional[dict]:
    ...
    Used to avoid re-submitting jobs.
    """
```
...
Update `_parse_job_response()` to parse the new `components` and `component_source` fields
into `AnalysisResult`. Add fields to the `AnalysisResult` dataclass:
...
3. Downloads LRC content from R2 via `R2Client.download_lrc_content(hash_prefix)` if
   `recording.has_lrc`. **(v3: method name `download_lrc_content` on the admin-cli
   R2Client is intentional; the analysis-service R2Client uses `download_lrc_text`.
   See Phase 2.6 note.)**
4. Checks R2 for cached `components.json` at `{hash_prefix}/components.json` (unless
   `force`). Verifies `schema_version` is current before reuse.
...
| `lrc_status='completed'` only | Pass `lrc_content` in the job request; worker runs `analyze_audio_fast()` inline for beats | **<1 s** identification + ~3 s fast_beats + ~10 s features |
...
(e.g., `sow-admin audio lrc`, download pipeline, or render path) is responsible for
generating LRC when needed.
...
**v3 migration note:** If deploying v3 on a database where v2 was already deployed (with
the OLD unique index `(song_id, component_type, occurrence_index)`), the upgrade path is:

```sql
-- Idempotent migration (wrap in a one-time migration block):
DROP INDEX IF EXISTS idx_song_components_unique;
CREATE UNIQUE INDEX IF NOT EXISTS idx_song_components_unique
  ON song_components(song_id, component_type, occurrence_index, role);
```

Document this in the next `db init`/migration script (or a one-off `sow-admin db migrate`
subcommand if v2 had been deployed in production). For greenfield deployments, the v3 DDL
is authoritative.
...
| `test_identify_from_allin1_single_chorus` | 1 chorus → returns TWO ComponentInstance rows (entry+exit), same start/end |
...
| `test_identify_from_lyrics_repetition_disambiguates_repeated_verse` | v3 — LRC where a verse repeats 4× and chorus 2×; algorithm picks chorus (mid-song position, lyrical cues) |
| `test_identify_from_lyrics_repetition_position_weight` | Candidate at song start penalized vs mid-song candidate with same repetition |
| `test_identify_from_lyrics_repetition_content_cue` | Candidate containing '赞美' wins tiebreak over candidate lacking chorus-typical vocabulary |
...
| `test_extract_components_lyrics_fallback_no_beats_invokes_fast_analyze` | v3 — tier-2 path with no beats calls analyze_audio_fast() (mocked); result saved to local cache |
| `test_extract_components_skip_no_data` | No sections, no lrc → returns `([], 'none')` |
| `test_extract_components_cache_hit_local` | Cached components.json in local cache with current schema_version → returns cached result immediately |
| `test_extract_components_cache_hit_r2` | No local cache, cached components.json in R2 with current schema_version → downloads and returns |
| `test_extract_components_cache_hit_stale_schema_version_skipped` | v3 — cached components.json with `schema_version=0` → treated as miss, recomputes |
| `test_extract_components_force_bypasses_cache` | force=True → skips cache, recomputes |
| `test_extract_components_cache_save_includes_schema_version` | After extraction, saved payload contains `schema_version=1` |

Use `numpy` synthetic audio (sine waves) for feature computation tests. Mock R2 client
and `analyze_audio_fast`.
...
| `test_song_components_unique_index_includes_role` | v3 — verify the unique index is `(song_id, component_type, occurrence_index, role)` |
| `test_song_components_role_check_includes_entry_exit_value` | role CHECK constraint allows 'entry_exit' value |
...
| `test_upsert_song_components_single_chorus_two_roles` | v3 — single-chorus song persists 2 rows (role='entry' + role='exit', same occurrence_index=1) |
...
| `test_get_song_components_entry_exit` | Returns (entry, exit) tuple; for single-chorus songs returns both rows |
...
| `test_components_command_uses_cached_r2_result` | Cached components.json in R2 with current schema_version → skips job submission |
| `test_components_command_cached_stale_schema_version_re_submits` | v3 — cached components.json with stale schema_version → re-submits |
...
| Component identification from LRC lyrics repetition (multi-cue) | <1 s |
| Inline `analyze_audio_fast` for tier-2 beats (when missing) | ~2–4 s |
...
Total for a typical backfilled song (tier 1): **~10–20 s** (mostly feature extraction +
optional audio download). For LRC-only songs (tier 2): **~15–25 s** (adds fast_analyze
inline when beats are missing). Songs with neither sections nor LRC are skipped.
...
|---|---|
| `allin1_sections` | 0.9 |
| `lyrics_repetition` (with beats) | 0.7 |
| `lyrics_repetition` (no beats, fast_analyze fallback failed, librosa-on-slice only) | 0.5 |
| `none` | 0.0 |
...
### v3 unique index proposal for review

The unique index `(song_id, component_type, occurrence_index, role)` means two rows for a
single-chorus song are valid (one entry, one exit, both occurrence_index=1). If a future
consumer prefers a single row indicating both roles, the schema already permits the
`role='entry_exit'` enum value (added in Phase 0.1); switch the persistence path to emit
one row instead of two without further migration.
...
| File | Description |
|---|---|
| `ops/analysis-service/src/sow_analysis/workers/components.py` | Component extraction: allin1-section identification, multi-cue lyrics-repetition clustering, per-component feature computation, cache-aware (defense-in-depth) extract_components() |
| `ops/analysis-service/tests/test_components.py` | Unit tests for identification algorithms, multi-cue disambiguation, feature computation, cache version handling |
| `ops/analysis-service/tests/integration/test_component_job.py` | Integration test for the component analysis job pipeline |
| `ops/admin-cli/tests/admin/test_client_components.py` | DB client tests for upsert/get song_components, including v3 single-chorus two-role case |
...
| File | Description |
|---|---|
| `ops/admin-cli/src/stream_of_worship/admin/db/schema.py` | Add `CREATE_SONG_COMPONENTS_TABLE` (with new `role='entry_exit'` enum value), v3 unique index `(song_id, component_type, occurrence_index, role)`, indexes, trigger, column constants; append to `ALL_SCHEMA_STATEMENTS` |
...
| `ops/admin-cli/src/stream_of_worship/admin/db/client.py` | Add `upsert_song_components()`, `get_song_components()`, `get_song_components_by_role()`, `get_song_components_by_type()`, `get_song_components_entry_exit()` |
| `ops/admin-cli/src/stream_of_worship/admin/services/analysis.py` | Add `submit_component_analysis()` to `AnalysisClient`; add `components`/`component_source` to `AnalysisResult`; update `_parse_job_response()`; add `get_cached_component_result()` for R2 cache check (with schema_version validation) |
...
| `ops/analysis-service/src/sow_analysis/config.py` | Add `SOW_COMPONENT_MAX_CONCURRENT` setting (cgroup-aware default) |
...
| `ops/analysis-service/src/sow_analysis/workers/queue.py` | Add `COMPONENT_ANALYSIS` dispatch in dedicated `_component_semaphore` block (NOT shared with `_fast_analyze_semaphore`); add `_process_component_analysis_job()` (with inline `analyze_audio_fast` fallback for tier-2 missing beats); add optional import of `components` module |
| `ops/analysis-service/src/sow_analysis/storage/cache.py` | Add `COMPONENT_SCHEMA_VERSION = 1`; add `get_component_result()` (with schema_version validation) / `save_component_result()` methods |
| `ops/analysis-service/src/sow_analysis/storage/r2.py` | Add `upload_component_result()` and `download_component_result()` methods (with schema_version validation); add `download_lrc_text()` method |
