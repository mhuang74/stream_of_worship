...
- **Platforms**: macOS, Linux, Windows (cross-platform support required).
...
- **Data Architecture**: Centralized song catalog via cloud storage (Cloudflare R2 or S3) for multi-device access and sharing with trusted collaborators.
- **Rendering Strategy**: Local by default (acceptable render times for background tasks), optional cloud for 4K or batch exports.
...
- **Phase 2 (Required)**: Cloudflare R2 or S3-compatible sync. S3-compatible API, zero egress fees. Required for centralized song catalog across devices and collaborators.
...
### 4.3 Stem Separation Configuration

**Stem Separation Tool**: Use Demucs (`htdemucs_6s` model) for high-quality stem separation.
- **Processing Time**: ~10-20 minutes per song on CPU, ~1-2 minutes on GPU.
- **Quality Priority**: Demucs provides superior stem quality compared to alternatives (e.g., Spleeter). The slower processing time is acceptable for offline batch ingestion.
- **Stems Generated**: `vocals.wav` (kept at 100% during transitions), `drums.wav`, `bass.wav`, `other.wav` (all faded logarithmically).

### 4.4 LRC Format (Enhanced)
...
*   **Render UX**: Blocking with progress bar (percentage). TUI is unresponsive during render. Video rendering runs as a background task - longer render times (10-20 minutes for a full set) are acceptable as user is not waiting on-screen.
*   **Text Rendering**: Uses Pillow for high-quality font rendering (gradients, shadows, outline). While slower than FFmpeg subtitles, the visual quality is prioritized for production worship videos.
...
## 9. Key Tech Stack Decisions & Rationale

### 9.1 ASR: Whisper large-v3
**Decision**: Use Whisper large-v3 model for all LRC generation.
**Rationale**:
- Worship lyrics require high accuracy; timing errors are disruptive during service.
- Processing happens offline during ingestion, so longer CPU times (1-2 min/song) are acceptable.
- GPU acceleration reduces to ~10-15 sec/song for those with hardware.
- Alternative models (base/medium) provide insufficient accuracy for worship context.

### 9.2 Stem Separation: Demucs htdemucs_6s
**Decision**: Use Demucs over faster alternatives like Spleeter.
**Rationale**:
- Vocals must remain clean at 100% during transitions; quality is critical.
- Demucs provides superior stem separation quality, especially for vocal isolation.
- Processing runs offline; 10-20 min/song on CPU is acceptable for batch ingestion.
- Faster alternatives (Spleeter) introduce artifacts audible during transitions.

### 9.3 Video Text Rendering: Pillow over FFmpeg Subtitles
**Decision**: Use Pillow for text overlay generation instead of FFmpeg's native subtitle filter.
**Rationale**:
- Worship videos benefit from professional styling (gradients, shadows, outlines, smooth transitions).
- FFmpeg subtitle filters have limited styling capabilities compared to Pillow's full text rendering.
- Render time (~10-15 min for 20-min playlist) is acceptable as a background task.
- Quality of final video is prioritized over render speed for production worship content.

### 9.4 LLM: GPT-4o-mini via OpenRouter
**Decision**: Use GPT-4o-mini for LRC alignment and metadata generation.
**Rationale**:
- Cost-effective (~$0.01-0.02 per song; $1-2 for 100-song catalog).
...
**Rationale**:
- Zero egress fees from R2 significantly reduce costs compared to AWS S3.
- Simple file-based architecture avoids database operation costs.
- Multi-device access required for user workflow.
- Docker distribution to trusted collaborators requires shared asset access.
...
- **Cache**: Platform-appropriate cache directories:
  - Linux: `~/.cache/stream_of_worship/` (XDG_CACHE_HOME)
  - macOS: `~/Library/Caches/StreamOfWorship/`
  - Windows: `%LOCALAPPDATA%\StreamOfWorship\cache\`
...
# Cache (Linux)
~/.cache/stream_of_worship/
└── whisper_cache/

# Cache (Windows)
%LOCALAPPDATA%\StreamOfWorship\cache\
└── whisper_cache\
```
