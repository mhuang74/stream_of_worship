...
| **New QuotaWaiter module** | New file: `workers/quota_waiter.py` | New class — core abstraction | Medium (~120 lines) |
| **MVSEP client** | `services/mvsep_client.py` | Expose `_quota_exhausted` vs `_disabled` distinction (minor property) | Trivial |
| **Qwen3 ASR client** | `services/qwen3_asr_client.py` | Add `_quota_exhausted` flag + UTC reset + new `Qwen3AsrQuotaExhaustedError` exception; modify `_raise_for_response` to expose status code; modify transcribe to raise QuotaExhaustedError when is_available is False due to quota | Medium |
| **LRC worker** | `workers/lrc.py` | Inject pre-created `Qwen3AsrClient` singleton or accept it as an optional parameter so QuotaWaiter probes the same instance | Low |
...
| **Job queue / LRC pipeline** | `workers/queue.py` | Conditional wait-or-fallback in `except Qwen3AsrQuotaExhaustedError` handler (must be placed BEFORE `except Qwen3AsrError`); QuotaWaiter injection via setter | Medium |
| **Application startup** | `main.py` | Create QuotaWaiter instances in lifespan, inject into queue; stop them in shutdown | Low |
...
| **Heartbeat** | Call `_update_stage` with identical values every 60 seconds to refresh `updated_at` so monitoring doesn't flag as stale. This is a DB write every 60s per waiting job, not every 1s. |
...
`self._update_stage` already accepts `(job, stage, progress)`. Since `_separate_with_mvsep_fallback` receives `job` as a direct argument, it would be cleaner to adjust the `StageUpdater` signature to include `job`, but the existing call sites in tests and other consumers expect `job` as a positional arg. The lambda approach at the call site keeps the stem-separation functions decoupled from the `JobQueue` instance.
...
8. **Add `is_available` check at top of `transcribe()`** (line 104-108): When `_quota_exhausted` is True and `_check_quota_reset()` does not clear it, raise `Qwen3AsrQuotaExhaustedError` immediately:
   ```python
   if self._quota_exhausted:
       self._check_quota_reset()
       if self._quota_exhausted:
...
CRITICAL: The `except` blocks must be ordered most-specific first. `Qwen3AsrQuotaExhaustedError` must come BEFORE `Qwen3AsrError`:
...
while True:
    try:
        ...
        if settings.SOW_FREE_ONLY_MODE:
            ...
            total_wait_seconds += time.time() - wait_start
            if not available or job.status == JobStatus.CANCELLED:
                return  # cancelled
...
This way, the 1800s budget only counts actual API processing time, not quota-wait time. The existing timeout/budget machinery in `_run_mvsep_stage_with_retries()` works unchanged. The same pattern is used for `_stage2_time_remaining()`.
...
- **Note**: The job restarts from the beginning of its pipeline (re-download, etc.). No additional recovery logic needed, but operators should be aware that long-waiting jobs will lose progress on restart.
...
|---|---|---|
| `workers/quota_waiter.py` | **New** | Medium — QuotaWaiter class (~120 lines) |
...
| `workers/queue.py` | Add QuotaWaiter + Qwen3Client setters; import `Qwen3AsrQuotaExhaustedError` with fallback; LRC wait-retry loop with correct except ordering; pass stage updater to stem separation | Medium |
| `main.py` | Create QuotaWaiter + Qwen3Client instances in lifespan, inject into queue; stop in shutdown | Low |
