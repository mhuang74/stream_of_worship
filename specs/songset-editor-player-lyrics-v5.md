---
# Songset Editor: Player-Integrated Lyrics Display — v5

[Supersedes v4 (`songset-editor-player-lyrics-v4.md`). Changes: icon switched from Mic2 to AlignLeft/FileText, 'L' keyboard shortcut added, scroll-chaining prevention added, global layout padding explicitly deferred out of scope, z-index/modal overlap documented as edge case, `useAudioPlayer.ts` changes clarified as optional API-completeness.]
...
| Aspect | v4 | v5 |
|--------|-----|-----|
| Write location | `specs/songset-editor-player-lyrics-v4.md` | This file |
| Lyrics toggle icon | `Mic2` (microphone) | `AlignLeft` (primary) or `FileText`; both are text/document icons that communicate "lyrics" more clearly than a microphone |
| Keyboard shortcut | None | **'L'** key toggles lyrics when the player bar is active and focus is not inside an input/textarea |
| Scroll chaining on lyrics panel | Not addressed | **`overscroll-y-contain`** prevents body scroll propagation when reaching top/bottom of lyrics on mobile |
| Global layout padding | Mentioned as acceptable gap | **Explicitly out of scope** — a future global layout refactor (e.g. CSS custom property `--player-height`) can address padded layouts across all pages |
| Z-index / modal overlap | Not addressed | Documented as **Edge Case 12** — expanded lyrics share the player's `z-[60]` and may visually sit above `z-50` modals/sheets |
| `useAudioPlayer.ts` changes | Required (Step 2) | **Optional / API-completeness only** — all v3→v4 call sites actually call `play()` directly from `useAudioPlayerContext`, but updating the hook wrappers keeps the public API consistent |
...
- **Synchronized auto-scroll is out of scope**: No line highlighting or auto-scroll based on playback `currentTime`. This is a static display (same as v3/v4). Future enhancement candidates: per-line highlighting via `findCurrentLyricIndex()`, click-to-seek on timestamp.
...
- **Global layout padding is out of scope**: Only `SongsetEditor` has `pb-24` bottom padding; `BrowseSheet` has `pb-28 sm:pb-20`. Other pages may lack bottom padding, causing the expanded lyrics panel (up to `40vh`) to hide bottom content. This is acceptable for v5 — users can collapse lyrics to reach content. A future global layout refactor should introduce a `--player-height` CSS custom property and update all page layouts accordingly.
...
| File | Role |
|------|------|
| `delivery/webapp/src/components/audio/AudioPlayerBar.tsx` | The Player bar (fixed bottom-0). Will gain the Lyrics toggle button + the lyrics panel + keyboard shortcut listener. Currently: 236 lines. |
...
| `delivery/webapp/src/hooks/useAudioPlayer.ts` | Exposes `playSong`, `playTransition`, `playLyricsLoop`. `PlaySongOptions` and `PlayLyricsLoopOptions` will gain an optional `recordingContentHash` field; it is forwarded to `AudioTrack`. **Note:** Current call sites use `play()` directly from context, so these changes are API-completeness only. |
...
| `delivery/webapp/src/components/songset/SongList.tsx` | Song row rendering. v3 added: `LyricsPanel` component, `expandedItemId` accordion state, `handleDragStart` auto-collapse, chevron button in `SortableSongItem`, `handleToggleExpand`. v4/v5 removes all of these. The `formatTimestamp()` local function at line 91 is moved to the new `PlayerLyricsPanel` component (or a shared util). |
| `delivery/webapp/src/components/songset/SongsetEditor.tsx` | Renders `<SongList>` and has `pb-24` bottom padding (line 396) to clear the fixed player bar. No direct changes. |
...
All song-starting call sites (`SongList`, `BrowseSheet`, `SemanticSearch`) already have `item.recording.contentHash` or `recording.contentHash` available in scope. They just don't currently pass it to `play()`. The `useAudioPlayer` hook's `playSong` wrapper also accepts `songId` and looks up the song — it needs to add the field to `PlaySongOptions` for API-completeness, even though current callers use `play()` directly from context.
...
> **Note:** This step is **API-completeness only**. All current call sites (`SongList`, `BrowseSheet`, `SemanticSearch`) call `play()` directly from `useAudioPlayerContext`, not through these wrapper functions. However, keeping the wrapper API consistent prevents future callers from forgetting the field.
...
#### 5c. Keyboard Shortcut ('L')

Add a document-level `keydown` listener (scoped behind a `useEffect`):

```typescript
useEffect(() => {
  const handleKeyDown = (e: KeyboardEvent) => {
    // Only respond when player is visible and focus is not inside an input/textarea
    ...
    if (e.key === "l" || e.key === "L") {
      e.preventDefault();
      // Only toggle if the current track supports lyrics
      if (currentTrack.recordingContentHash) {
...
- Uses `e.preventDefault()` to avoid conflicts with browser find (Ctrl+F on some layouts) or other 'L' listeners.
...
- **`title="Lyrics (L)"`** — shows the keyboard shortcut on hover via native tooltip.

Import: `AlignLeft` from `lucide-react` (add to the existing `lucide-react` import destructure on line 6). `FileText` is an acceptable alternative if `AlignLeft` feels ambiguous.

#### 5e. Render the Lyrics Panel
...
> **Note on `overscroll-y-contain`**: Added to the inner scrollable region to prevent mobile scroll chaining into the page body when the user reaches the top or bottom of the lyrics.

#### 5g. Import the New Component
...
#### 5h. Destructure `useState`, `useEffect`
...
> **Out of scope (deferred):** A global `--player-height` CSS custom property that dynamically reflects the player's current height (collapsed or expanded) and is consumed by all page layouts. This would prevent hidden content on pages without bottom padding (e.g., home page, settings). Tackled in a future layout refactor.
...
| File | Purpose |
|------|---------|
| `delivery/webapp/src/components/audio/PlayerLyricsPanel.tsx` | Lyrics rendering component for the AudioPlayerBar. Uses `useSongLyrics` + LRC parser. Adds `overscroll-y-contain` to prevent scroll chaining. |
...
| `delivery/webapp/src/hooks/useAudioPlayer.ts` | Add `recordingContentHash?: string` to `PlaySongOptions` + `PlayLyricsLoopOptions`; forward to `AudioTrack`. (API-completeness; not on the hot migration path.) |
| `delivery/webapp/src/components/audio/AudioPlayerBar.tsx` | Add `showLyrics` state + auto-collapse effect + 'L' keyboard shortcut + Lyrics toggle button (`AlignLeft`) + animated lyrics panel wrapper + import `PlayerLyricsPanel`. |
...
## Files NOT Modified (Reused as-is from v3/v4)
...
```typescript
import { useState, useEffect } from "react";
import { AlignLeft } from "lucide-react";  // add to existing destructure (FileText is acceptable alternative)
import { PlayerLyricsPanel } from "./PlayerLyricsPanel";
```
...
7. **Mobile: lyrics panel is very tall**: `max-h-[40vh]` caps the height. The panel scrolls internally. The seek bar and controls remain accessible below. `overscroll-y-contain` prevents accidental body scroll.
...
10. **User navigates away from the songset page**: The player is global and persists across navigations (within the app). If a song is playing and lyrics are open, both the player and lyrics remain visible on other pages. **Known limitation:** pages without bottom padding may have content hidden behind the expanded panel. The user can collapse lyrics or scroll to reach content.
...
12. **Modal/sheet opened while lyrics panel is expanded**: The player container uses `z-[60]`. Standard shadcn/ui modals and sheets typically use `z-50`. An expanded lyrics panel could render above a modal backdrop if the user opens a modal while music plays. This is acceptable for v5 — the user can collapse lyrics to clear the view. Future work may coordinate z-index or auto-collapse lyrics when a modal opens.
...
10. Verify on mobile (`< 768px`): timestamps stack above lyric text for LRC content; panel has `max-h-[40vh]` with internal scroll; scroll chaining does not propagate to body.
...
15. Press 'L' while player is visible → verify lyrics toggle.
16. Focus a text input on the page, press 'L' → verify lyrics do NOT toggle.
...
- **Global layout bottom padding / `--player-height` CSS variable**: A future refactor should make all page bottoms aware of the player's current height so content is never hidden. Deferred.
...
5. **Player enrichment**: Add the Lyrics button (`AlignLeft`) + panel + 'L' shortcut + `overscroll-y-contain`. This is purely additive — if `showLyrics` is never set to `true`, the player behaves identically to before.
