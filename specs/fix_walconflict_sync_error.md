...
The app-side `AppSyncService` has a similar sync-with-recovery pattern. Add the same WalConflict recovery condition to its `_execute_sync_with_recovery()` method (if it has one), or to the sync error handling in its `execute_sync` method.
