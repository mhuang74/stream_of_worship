...
playback preferences, and Postgres connection configuration.
"""
...
class AppConfig:
    ...
    Note:
        Postgres password is read from SOW_DATABASE_PASSWORD environment variable
        only (not stored in config file for security).
    """
    ...
    # Postgres connection (password via SOW_DATABASE_PASSWORD env var only)
    database_url: str = ""
    ...
    def load(cls, path: Optional[Path] = None) -> "AppConfig":
        ...
        # Load Postgres database config — new [database] section
        if "database" in data:
            db = data["database"]
