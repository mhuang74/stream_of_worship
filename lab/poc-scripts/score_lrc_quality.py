...
Usage:
    PYTHONPATH=src:. uv run --extra score_lrc_base python score_lrc_quality.py \
...
def resolve_stem_path(
    ...
    if vocals_stem_path.exists():
        typer.echo(f"Using cached vocals stem: {vocals_stem_path}", err=True)
        return vocals_stem_path

    # Try main audio as last resort
...
def main(
    ...
    from sow_legacy_cli_tui.core.paths import get_cache_dir as _get_cache_dir
