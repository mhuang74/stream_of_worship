...
Computes phase/theme/signal-coverage metrics for the enriched candidate pool
and renders both a markdown artifact and a console-friendly summary. Used by
the ``--only-evaluate-pool-enrichment`` CLI flag (see spec
``songset-constructor-traditional-chinese-and-enrich-pool-eval-v1.md``).
"""
...
BAR_SCALE = 0.04  # one ▓ block per ~2.5 percentage points
...
def build_enrichment_report(
    *,
    ...
    """Compute enrichment metrics and render the markdown report.

    Returns a ``(metrics, report_text)`` tuple where ``metrics`` is a
    JSON-serializable dict consumed by the CLI console summary and
    ``report_text`` is the markdown written to ``enrichment_report.md``.
    """
    ...
    theme_inferred = sum(
        1
        for candidate in pool
        if candidate.themes and max(candidate.themes.values(), default=0.0) > 0
    )
    tempo_fallback = enriched_size - theme_inferred
    tempo_fallback_no_tempo = sum(
        1
        for candidate in pool
        if (not candidate.themes or max(candidate.themes.values(), default=0.0) == 0.0)
        and candidate.tempo_bpm is None
    )
    ...
    for candidate in pool:
        ...
        if any(value > 0 for value in title.values()):
            title_hits += 1
        lyrics = classify_lyrics_themes(candidate.lyrics_raw)
        if any(value > 0 for value in lyrics.values()):
            lyrics_hits += 1
        if candidate.song_embedding:
            song_emb_hits += 1
        if candidate.line_embeddings:
            line_emb_hits += 1

    tempo_known = sum(1 for candidate in pool if candidate.tempo_bpm is not None)
    tempo_missing = enriched_size - tempo_known
    bpms = sorted(
        candidate.tempo_bpm for candidate in pool if candidate.tempo_bpm is not None
    )
    ...
    album_series_counts: Counter[str | None] = Counter(
        candidate.album_series for candidate in pool
    )
    ...
    metrics: dict[str, Any] = {
        ...
        "enriched_size": enriched_size,
        "dropped": dropped,
        "drop_reasons": dict(drop_reasons),
        "dropped_samples": list(dropped_samples),
        "phase_distribution": phase_distribution,
        "theme_distribution": theme_distribution,
        "zero_theme_count": zero_theme_count,
        "theme_inferred": theme_inferred,
        "tempo_fallback": tempo_fallback,
        "tempo_fallback_no_tempo": tempo_fallback_no_tempo,
        "signal_coverage": {
            "title_hits": title_hits,
            "lyrics_hits": lyrics_hits,
            "song_embedding": song_emb_hits,
            "line_embeddings": line_emb_hits,
        },
        "tempo_key_coverage": {
            "tempo_known": tempo_known,
            ...
            "low_confidence_key": low_confidence_key,
        },
        "album_series_distribution": [
            {"series": series, "count": count}
            for series, count in album_series_counts.most_common()
        ],
        "diversity": {
            "unique_themes_covered": unique_themes_covered,
...
def _render_markdown(metrics: dict[str, Any], config: RunConfig) -> str:
    ...
    lines.append(f"- Pool limit: {config.pool_limit}")
    ...
    if metrics["drop_reasons"]:
        for reason, count in sorted(
            metrics["drop_reasons"].items(), key=lambda item: (-item[1], item[0])
        ):
    ...
    if metrics["dropped_samples"]:
        ...
        for sample in metrics["dropped_samples"]:
            lines.append(
                f"- {sample.get('title', '?')} (`{sample.get('recording_hash_prefix', '?')}`): "
                f"{sample.get('reason', '?')}"
            )
    lines.append("")
    ...
    for entry in metrics["theme_distribution"]:
        ...
        lines.append(
            f"| {entry['theme']} | {entry['count']} | {entry['pct']:.1f}% | {status} |"
        )
    ...
    lines.append(
        f"- Tempo fallback with no tempo (default phase 3): **{metrics['tempo_fallback_no_tempo']}**"
    )
    lines.append("")
    ...
    if tk["bpm_min"] is not None:
        bpm_summary = (
            f"min={tk['bpm_min']:.1f}, max={tk['bpm_max']:.1f}, median={tk['bpm_median']:.1f}"
        )
    ...
    lines.append(
        f"- Theme entropy: {div['theme_entropy']:.3f} bits "
        f"(max {div['max_theme_entropy']:.3f})"
    )
    lines.append(
        f"- Phase entropy: {div['phase_entropy']:.3f} bits "
        f"(max {div['max_phase_entropy']:.3f})"
    )
    lines.append("")
...
def render_console_summary(metrics: dict[str, Any], report_path: str) -> str:
    """Render the spec's console summary block as a single string."""
    ...
    lines.append(
        f"Pool: {metrics['loaded_size']} loaded → {metrics['enriched_size']} enriched "
        f"({metrics['dropped']} dropped{drop_reason})"
    )
    lines.append("")
    lines.append("Phase Distribution:")
    for entry in metrics["phase_distribution"]:
    ...
    sorted_themes = sorted(
        metrics["theme_distribution"], key=lambda entry: (-entry["count"], entry["theme"])
    )
    for entry in sorted_themes:
        marker = " ← underrepresented" if entry["underrepresented"] else ""
        lines.append(
            f"  {entry['theme']:<6} {entry['count']:>4} songs ({entry['pct']:>5.1f}%){marker}"
        )
    lines.append("")
    lines.append(
        f"Phase Inference: {metrics['theme_inferred']} from themes, "
        f"{metrics['tempo_fallback']} tempo-only fallback "
        f"({metrics['tempo_fallback_no_tempo']} with no tempo)"
    )
    signal = metrics["signal_coverage"]
    total = metrics["enriched_size"]
    lines.append(
        f"Signal Coverage: title {signal['title_hits']}/{total}, "
        f"lyrics {signal['lyrics_hits']}/{total}, "
        f"embeddings {signal['song_embedding']}/{total}"
    )
    div = metrics["diversity"]
    lines.append(
        f"Theme Entropy: {div['theme_entropy']:.2f} bits (max {div['max_theme_entropy']:.2f})"
    )
    lines.append(
        f"Phase Entropy: {div['phase_entropy']:.2f} bits (max {div['max_phase_entropy']:.2f})"
    )
    lines.append("")
