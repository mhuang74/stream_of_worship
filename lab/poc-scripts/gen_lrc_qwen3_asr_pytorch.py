#!/usr/bin/env python3
"""Qwen3-ASR PyTorch backend POC script.

This script provides a standalone POC for the PyTorch Qwen3-ASR backend,
allowing side-by-side comparison with the ONNX backend.

Features:
- Per-character timestamps (same as MLX backend)
- Context biasing support for better transcription
...
app = typer.Typer(help="Qwen3-ASR PyTorch backend POC with per-character timestamps")


def transcribe_pytorch(
    audio_path: Path,
    ...
    Returns:
        Raw transcription result as dict with text, language, and segments
    """
    import torch
    ...
    typer.echo(f"Using device: {device}, dtype: {dtype}", err=True)
    ...
    # Convert to dict format with per-character timestamps
    result = results[0]
    raw_result = {
        "text": result.text,
    ...
    # Extract per-character timestamps if available
    time_stamps = getattr(result, "time_stamps", None)
    if time_stamps:
        # time_stamps is a list of (start, end) tuples for each character
        text_chars = list(result.text)
...
def extract_segments(raw: dict) -> list[dict]:
    """Extract phrase-level segments from per-character ASR output.

    Reconstructs phrase-level segments from per-character timestamps
    by splitting on Chinese and ASCII punctuation.

    Args:
        raw: Raw ASR output dict with 'text' and 'segments'
    ...
    if len(char_segments) != non_punct_char_count:
        typer.echo(
            ...
            f"found {len(char_segments)} segments). Using per-character segments.",
            err=True,
        )
        return char_segments

    # Reconstruct phrase-level segments
    segments = []
    seg_idx = 0
    ...
    while seg_idx < len(char_segments):
        ...
        while seg_idx < len(char_segments):
            ...
            if phrase_start < len(text) and text[phrase_start] in punct_set:
...
def cache_file_name(
    ...
    filename = f"pytorch_{safe_song_id}_{safe_model}_{params_hash}.json"
...
def canonical_line_snap(
    ...
    Implements greedy algorithm for matching ASR segments to canonical lyrics.
    ...
    # Find first content segment (skip filler at start)
    first_content_idx = 0
...
def write_diagnostic(
    ...
    lines = []
    lines.append("# Qwen3-ASR PyTorch Backend Diagnostic Report\n")
...
def main(
    ...
    model: str = typer.Option(
        "Qwen/Qwen3-ASR-0.6B",
        "--model",
        help="Model name (Qwen/Qwen3-ASR-0.6B or Qwen/Qwen3-ASR-1.7B)",
    ),
    ...
    """Run Qwen3-ASR PyTorch transcription on a song and output LRC format.

    This script uses the qwen-asr PyTorch backend which provides per-character
timestamps, enabling better alignment with canonical lyrics.

    Transcription results are cached and reused by default. Use --force-rerun
to ignore the cache.
    """
    # Validate model
    if model not in ("Qwen/Qwen3-ASR-0.6B", "Qwen/Qwen3-ASR-1.7B"):
        typer.echo(f"Error: Invalid model '{model}'. Use 'Qwen/Qwen3-ASR-0.6B' or 'Qwen/Qwen3-ASR-1.7B'.", err=True)
        raise typer.Exit(1)
    ...
    cache_path = cache_file_name(cache_dir, song_id, model, params_hash)
    ...
    if raw is None:
        ...
        try:
            ...
            # Transcribe with PyTorch backend
            raw = transcribe_pytorch(
            ...
            # Save to cache
            save_cached_transcription(cache_path, raw, model, params, wall_time)
    ...
    if save_raw:
        ...
        raw_file = save_raw / "asr_raw_pytorch.json"
    ...
    if snap:
        ...
        if save_raw:
            save_raw.mkdir(parents=True, exist_ok=True)
            diag_file = save_raw / "diagnostic_pytorch.md"
            write_diagnostic(
