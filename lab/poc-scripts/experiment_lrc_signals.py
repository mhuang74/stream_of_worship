...
Usage:
    ...
    PYTHONPATH=src:. uv run --extra score_lrc_base,poc_qwen3_align \
        python experiment_lrc_signals.py

    # Without qwen3-forcedaligner (score_lrc_base only):
    PYTHONPATH=src:. uv run --extra score_lrc_base \
        python experiment_lrc_signals.py --skip-qwen3

    # Single song:
    PYTHONPATH=src:. uv run --extra score_lrc_base,poc_qwen3_align \
        python experiment_lrc_signals.py --song dan_dan_ai_mi_249

    # Skip slow signals:
    PYTHONPATH=src:. uv run --extra score_lrc_base,poc_qwen3_align \
        python experiment_lrc_signals.py --skip-dtw --skip-tone
