...
```bash
pytest lab/sow-app/tests/test_navigation.py -v
```

### Run Specific Test
```bash
pytest lab/sow-app/tests/test_navigation.py::TestNavigationFlow::test_visual_freeze_detection -v
```

### Save Screenshots for Visual Inspection
```bash
pytest lab/sow-app/tests/test_navigation.py --tmp-path=/tmp/nav-test -v
```
