...
checking database connectivity, and managing songset exports/imports.
"""
...
from sow_lab_app.app import SowApp
from sow_lab_app.config import AppConfig, ensure_app_config_exists, get_app_config_path
...
from sow_lab_app.logging_config import setup_logging
from sow_lab_app.services.catalog import CatalogService
from sow_lab_app.services.songset_io import ImportResult, SongsetIOService
from sow_lab_app.services.sync import check_database_connection
...
def _check_database_connection(config: AppConfig) -> bool:
    ...
    try:
        ...
        console.print(
            Panel.fit(
                ...
                "Set database.url in your config or SOW_DATABASE_URL env var.",
                title="Error",
                border_style="red",
            )
        )
    ...
    if not check_database_connection(url):
        console.print(
            Panel.fit(
                "[bold red]Database unreachable![/bold red]\n\n"
                "Could not connect to the configured database.\n"
                "Check your network and database URL configuration.",
                title="Error",
                border_style="red",
            )
        )
...
def run(
    ...
    if not _check_database_connection(config):
        raise typer.Exit(1)
...
def db_check(
    ...
    if check_database_connection(url):
