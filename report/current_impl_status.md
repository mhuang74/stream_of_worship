...
- Implemented `specs/fix-android-video-no-render-h264-444-profile-v3.md` (Android video blank playback — H.264 High 4:4:4 profile + software fallback gap).
  - Phase 1 (user-driven, no code change): re-render of job `ccWknb1cqyD9s1H5qvDJM` is user-triggered via webapp UI; the current render-worker (`delivery/render-worker/src/sow_render/video_engine.py:129-145`) already emits `-profile:v high -pix_fmt yuv420p` (codec string `avc1.640028`), which Pixel 6's `c2.exynos.h264.decoder` decodes correctly. The broken `avc1.F40028` (High 4:4:4 Predictive) artifact in R2 predates the existing High-profile + anti-regression guard at `delivery/render-worker/tests/test_mp4_cast_compatibility.py:107-111`.
  - Phase 2.1 (FFmpeg software-decoder extension) — **NOT IMPLEMENTED**: the spec directed adding `implementation("androidx.media3:media3-decoder-ffmpeg:1.5.1")` to `delivery/android/app/build.gradle.kts`, but this artifact does not exist on Maven. Verification via the official `https://dl.google.com/dl/android/maven2/androidx/media3/group-index.xml` confirms there are **zero** `media3-decoder-*` extension artifacts published at any version (no ffmpeg, av1, vp9, opus, flac). Every Media3 software decoder extension is source-only and must be built locally with the NDK + native libraries (see `https://github.com/androidx/media/blob/main/libraries/decoder_ffmpeg/README.md`). Adding the non-existent Maven coordinate breaks the Gradle build with "Could not find androidx.media3:media3-decoder-ffmpeg:1.5.1". Deferred to a separate source-build effort; tracked as a known hardening gap. The existing `setEnableDecoderFallback(true)` + `EXTENSION_RENDERER_MODE_PREFER` flags at `VideoExoPlayerFactory.kt:30-33` remain in place (correct, but with no extension renderer to prefer until the source build lands).
  - Phase 2.2 (rewrite `VideoExoPlayerFactoryTest`): reverted to the existing flag-reflection assertions. The spec's stronger assertion (at least one video `Renderer` is `androidx.media3.decoder.ffmpeg.LiboffmpegVideoRenderer`) is not possible without the FFmpeg extension on the classpath and would always fail. Kept `setEnableDecoderFallback(true)` + `EXTENSION_RENDERER_MODE_PREFER` flag assertions as secondary assertions per the spec. When the FFmpeg source build lands, the renderer-class assertion should be added back.
  - Phase 2.3 (null out `playerViewState` in `DirectPlayerFacade.release()`): `Media3PlayerController.kt` — introduced a private `mutablePlayerViewState: MutableStateFlow<Player?>` field backing the read-only `playerViewState: StateFlow<Player?>`, and `DirectPlayerFacade.release()` now sets `mutablePlayerViewState.value = null` (mirroring `ServiceMediaControllerFacade.release()` at line 394). This clears the dead `ExoPlayer` reference that previously lingered as `videoPlayer` in `PlayerScreen.kt:73-74` until the controller was re-keyed. Used the private-field approach (preferred per spec note) rather than casting through `MutableStateFlow`.
  - Acceptance: `./gradlew testDebugUnitTest --tests "org.streamofworship.android.feature.player.VideoExoPlayerFactoryTest" --tests "...Media3PlayerControllerTest" --tests "...PlayerViewModelTest"` green; `./gradlew lintDebug assembleDebug` green. Deploy to Pixel 6 via `delivery/android/scripts/deploy_debug.sh`.
