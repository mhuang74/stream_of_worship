...
```bash
mkdir -p services/render-worker/secrets
cat > services/render-worker/secrets/r2_credentials <<EOF
...
- Ensure `services/render-worker/secrets/` is ignored (add if not present)
